...
DEFAULT_MEMORY_CACHE_SIZE = int(
    os.getenv("MEMORY_CACHE_SIZE", "1000")
...
class CacheService:
    ...
    def get(self, key_hash: str) -> Optional[CacheEntry]:
        ...
        # Level 2: Redis cache
        if (
            self.enable_multi_level
            and self.redis_cache
            and self.redis_cache.is_available()
        ):
    ...
    async def get_async(self, key_hash: str) -> Optional[CacheEntry]:
        ...
        try:
            ...
            try:
                payload = (
                    json.loads(row["payload_json"]) if row.get("payload_json") else None
                )
            except Exception:
                payload = None
            ...
            return CacheEntry(
                ...
                ttl_seconds=(
                    int(row["ttl_seconds"]) if row.get("ttl_seconds") is not None else 0
                ),
                source=row["source"],
                metadata_json=metadata,
                payload_json=payload,
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        if self.redis_cache:
            stats["redis_cache"] = {
                ...
                "hit_rate": (
                    self.redis_cache.hit_rate()
                    if hasattr(self.redis_cache, "hit_rate")
                    else 0
                ),
        ...
        # Overall stats
        total_hits = stats["memory_cache"].get("hits", 0) + stats["redis_cache"].get(
            "hits", 0
        )
        total_requests = stats["memory_cache"].get("total_requests", 0) + stats[
            "redis_cache"
    ...
    def warm_cache_from_patterns(
        self, usage_patterns: Dict[str, int], max_items: int = 100
    ) -> Dict[str, Any]:
        ...
        sorted_patterns = sorted(
            usage_patterns.items(), key=lambda x: x[1], reverse=True
        )
    ...
    def start_background_warmer(self, interval_seconds: int = 300) -> None:
        ...
        if (
            hasattr(self, "_warming_thread")
            and self._warming_thread
            and self._warming_thread.is_alive()
        ):
            ...
            while True:
                try:
                    ...
                    if overall_hit_rate < 0.7:  # Less than 70% hit rate
                        print(
                            f"🔄 Low cache hit rate ({overall_hit_rate:.2%}), triggering cache warming..."
                        )
    ...
    def get_warming_config(self) -> Dict[str, Any]:
        ...
        return {
            "enable_background_warmer": getattr(
                self, "enable_background_warmer", False
            ),
    ...
    def configure_warming(
        ...
        print(
            f"✅ Cache warming configured: enabled={enable_background_warmer}, interval={warming_interval_seconds}s"
        )
    ...
    def invalidate_by_pattern(self, pattern: str) -> int:
        ...
        if invalidated_count > 0:
            print(
                f"🗑️  Invalidated {invalidated_count} cache entries matching pattern: {pattern}"
            )
    ...
    def invalidate_with_dependencies(self, key: str) -> int:
        ...
        if invalidated_count > 0:
            print(
                f"🗑️  Invalidated {invalidated_count} cache entries with dependencies for: {key}"
            )
    ...
    def smart_invalidate(self, key: str, reason: str = "manual") -> Dict[str, Any]:
        ...
        if hasattr(self, "_reverse_dependencies"):
            result["dependencies_invalidated"] = len(
                self._reverse_dependencies.get(key, set())
            )
        ...
        print(
            f"🧠 Smart invalidation completed for {key}: {invalidated_count} entries invalidated"
        )
        return result
            ...
            metadata.update(
                {
                    "compression": {
                        ...
                        "optimized_size": (
                            len(str(optimized_data).encode("utf-8"))
                            if optimized_data
                            else 0
                        ),
        ...
        try:
            ...
            if USE_POOL and DatabasePool is not None:
                with DatabasePool.get_connection(self.db_path) as con:
                    cur = con.cursor()
                    cur.execute(
                        "SELECT metadata_json FROM cache_entries WHERE metadata_json IS NOT NULL"
                    )
                    rows = cur.fetchall()
            else:
                con = sqlite3.connect(self.db_path)
            ...
            for row in rows:
                try:
                    ...
                    if compression_info.get("enabled", False):
                        ...
                        stats["total_original_size"] += compression_info.get(
                            "original_size", 0
                        )
                        stats["total_optimized_size"] += compression_info.get(
                            "optimized_size", 0
                        )
            ...
            if stats["total_original_size"] > 0:
                ...
                stats["total_compressed_size"] = int(
                    stats["total_optimized_size"] * 0.7
                )  # Rough estimate
                stats["compression_ratio"] = (
                    stats["total_compressed_size"] / stats["total_original_size"]
                )
                stats["storage_savings_percent"] = (
