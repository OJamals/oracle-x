...
class FINRAAtsAdapter:
    ...
    def __init__(self):
        ...
        self.session.headers.update({
            'User-Agent': 'Oracle-X-Trading-Intelligence/1.0'
        })
    ...
    def _fetch_ticker_ats_data(self, ticker: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            logger.info(f"Fetching FINRA ATS data for {ticker} (week ending {week_date})")
    ...
    def _analyze_ats_activity(self, df: pd.DataFrame, ticker: str) -> Dict[str, Any]:
        ...
        try:
            ...
            ats_market_share = (total_ats_volume / total_market_volume * 100) if total_market_volume > 0 else 0
            
            # Get top venues by volume
            top_venues = df.nlargest(3, 'ATSVolume')[['VenueName', 'ATSVolume']].to_dict('records')
            
            # Generate signal based on ATS market share
            signal, confidence = self._generate_signal(ats_market_share, total_ats_volume)
            ...
            return {
                ...
                'top_venue_volumes': {v['VenueName']: int(v['ATSVolume']) for v in top_venues},
                'signal': signal,
    ...
    for ticker in tickers[:10]:
        try:
            ...
            if not intraday.empty:
                volume_threshold = avg_volume * 0.01  # 1% of daily avg in 5min = large trade
            ...
            dark_pool_probability = min(
                ...
                (large_trades / 78) * 0.3 +    # Large trade component (78 5min periods in day)
            ...
            if dark_pool_probability > 0.4:  # Lowered threshold from 0.5
                ...
                estimated_block_size = int((recent_volume - avg_volume) * 0.15)  # 15% of excess
                ...
                results[ticker] = {
                    ...
                    'signal': 'possible_institutional_activity' if dark_pool_probability > 0.6 else 'elevated_volume',
...
def fetch_dark_pool_data(tickers: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if finra_data.get('total_tickers_analyzed', 0) > 0:
            logger.info(f"Using FINRA ATS data for {len(finra_data['tickers'])} tickers")
            return finra_data
    
    except Exception as e:
        logger.warning(f"FINRA ATS data unavailable, using volume analysis fallback: {e}")
...
if __name__ == "__main__":
    ...
    print(f"Tickers Analyzed: {data.get('total_signals', data.get('total_tickers_analyzed', 0))}")
