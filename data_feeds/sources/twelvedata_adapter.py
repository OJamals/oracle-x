...
try:
    from core.http_client import get_http_client_manager
except ImportError:
    get_http_client_manager = None
...
class TwelveDataAdapter:
    ...
    async def _async_request(self, path: str, params: Dict[str, Any]) -> Dict[str, Any]:
        """Async version of _request using AsyncHTTPClient for concurrent operations."""
    ...
    async def get_quote_async(self, symbol: str) -> Optional[Any]:
        """Async version of get_quote for concurrent quote fetching."""
        data = await self._async_request("/quote", {"symbol": symbol})
        if not isinstance(data, dict):
            return None
    ...
    async def get_market_data_async(
        ...
        data = await self._async_request("/time_series", params)
        # Expected structure: {"values":[{"datetime":"...","open":"...","high":"...","low":"...","close":"...","volume":"..."}], ...}
        values: List[Dict[str, Any]] = []
        if isinstance(data, dict):
            values = data.get("values") or []
