...
class GrokTwitterAdapter:
    ...
    def fetch_tweets(self, query: str, limit: int = 20) -> str:
        ...
        try:
            ...
            texts = [
                {"text": tweet.get("text", ""), "tickers": tweet.get("tickers", [])}
                for tweet in tweets
            ]
...
Do not respond with anything else.""",
    ...
    def _run_agent(self, symbol: str, limit: int) -> Dict[str, Any]:
        ...
        for iteration in range(max_iterations):
            try:
                logger.info(
                    f"Agent iteration {iteration + 1}/{max_iterations}: calling OpenAI API"
                )
                    ...
                    for tool_call in message.tool_calls:
                        if tool_call.function.name == "fetch_tweets":
                            ...
                            tweets_json = self.fetch_tweets(
                                args.get("query", symbol), args.get("limit", limit)
                            )
                            messages.append(
                                {
                                    "role": "tool",
    ...
    def get_sentiment(self, symbol: str, limit: int = 20) -> Optional[SentimentData]:
        ...
        try:
            ...
            logger.info(
                f"Grok Twitter sentiment for {symbol}: {agent_result.get('sentiment', 0):.3f} "
