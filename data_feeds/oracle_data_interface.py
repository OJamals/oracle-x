...
class OracleDataProvider:
    ...
    def get_comprehensive_market_intelligence(self, tickers: Optional[List[str]] = None) -> MarketIntelligence:
        """
        ...
        for ticker in tickers:
            ...
            if quote and quote.quality_score and quote.quality_score >= self.min_quality_threshold:
                market_data[ticker] = {
    ...
    def get_earnings_calendar(self, tickers: Optional[List[str]] = None) -> List[Dict[str, Any]]:
        """
    ...
    def get_sentiment_analysis(self, tickers: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        for ticker in tickers:
            ...
            if sentiments:
                ...
                if ticker_sources > 0:
                    ...
                    valid_sentiments += 1
                    quality_scores.append(avg_confidence * 100)  # Convert confidence to quality score
        ...
        if valid_sentiments > 0:
            ...
            sentiment_summary["quality_score"] = sum(quality_scores) / len(quality_scores)
...
def fetch_dark_pool_data(tickers: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "high_volume_stocks": get_oracle_provider()._calculate_technical_indicators(
            get_oracle_provider().get_comprehensive_market_intelligence(tickers).market_data
        )["high_volume_stocks"]
    }
...
def fetch_earnings_calendar(tickers: Optional[List[str]] = None) -> List[Dict[str, Any]]:
    """Backward compatible earnings calendar (replaces placeholder)"""
    return get_oracle_provider().get_earnings_calendar(tickers)

def get_signals_from_scrapers_v2(prompt_text: str, chart_image_b64: Optional[str] = None) -> Dict[str, Any]:
    """
