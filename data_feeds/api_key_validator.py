...
class APIKeyValidator:
    ...
    def execute_with_fallback(self, service_name: str,
        ...
        if fallback_func and validation.get('fallback_available', False):
            logger.info(f"Using fallback for {service_name}: {validation.get('fallback_description', '')}")
            return fallback_func(*args, **kwargs)
        else:
            raise ValueError(f"API key required for {service_name}: {validation.get('reason', 'Invalid API key')}")
    ...
    def print_validation_report(self) -> None:
        ...
        # Missing keys
        missing_services = [(s, v) for s, v in validation['services'].items() if not v['valid']]
        if missing_services:
            ...
            for service, validation in missing_services:
                ...
                if validation.get('fallback_available'):
                    print(f"      Fallback: {validation.get('fallback_description', 'Available')}")
...
def execute_with_fallback(service_name: str, primary_func: Callable, fallback_func: Optional[Callable] = None, *args, **kwargs) -> Any:
    """Execute a function with automatic fallback if API key is invalid"""
    return _api_key_validator.execute_with_fallback(service_name, primary_func, fallback_func, *args, **kwargs)
