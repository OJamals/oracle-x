...
class RateLimiter:
    ...
    def wait_if_needed(self, source: DataSource):
        ...
        # Clean old calls
        self.calls[source] = [call_time for call_time in self.calls[source] 
...
class YFinanceAdapter(SourceAdapter):
    ...
    def get_historical(self, symbol: str, period: str = "1y") -> Optional[pd.DataFrame]:
        ...
        return self._get_cached_or_fetch(f"historical_{symbol}_{period}", "historical_daily", fetch)
    ...
    def get_company_info(self, symbol: str) -> Optional[CompanyInfo]:
        ...
        return self._get_cached_or_fetch(f"company_info_{symbol}", "company_info", fetch)
        ...
        def fetch():
            ...
            if quote_data and 'c' in quote_data:
                return Quote(
                    ...
                    market_cap=profile.get('marketCapitalization') * 1000000 if profile.get('marketCapitalization') else None,
                    day_low=Decimal(str(quote_data.get('l', 0))) if quote_data.get('l') else None,
                    day_high=Decimal(str(quote_data.get('h', 0))) if quote_data.get('h') else None,
                    timestamp=datetime.now(),
                    source=self.source.value
                )
            return None
        ...
        def fetch():
            ...
            if data and len(data) > 0:
                ...
                return Quote(
                    ...
                    pe_ratio=Decimal(str(item.get('pe', 0))) if item.get('pe') else None,
                    ...
                    year_low=Decimal(str(item.get('yearLow', 0))) if item.get('yearLow') else None,
                    year_high=Decimal(str(item.get('yearHigh', 0))) if item.get('yearHigh') else None,
                    timestamp=datetime.now(),
                    source=self.source.value
                )
            else:
                logger.warning(f"FMPAdapter.get_quote: No data returned for symbol '{symbol}'. Response: {data}")
            return None
        return self._get_cached_or_fetch(f"quote_{symbol}", "quote", fetch)
    ...
    def get_company_info(self, symbol: str) -> Optional[CompanyInfo]:
        def fetch():
            ...
            if data and len(data) > 0:
                ...
                logger.warning(f"FMPAdapter.get_company_info: No data returned for symbol '{symbol}'. Response: {data}")
            return None
        return self._get_cached_or_fetch(f"company_info_{symbol}", "company_info", fetch)
    
    def get_historical(self, symbol: str, from_date: Optional[str] = None, to_date: Optional[str] = None) -> Optional[pd.DataFrame]:
        def fetch():
            ...
            try:
                ...
                if hist_data and len(hist_data) >= 1:
                    ...
                    if isinstance(current_data, dict):
                        ...
                        current_price = current_data[3] if len(current_data) > 3 else current_data[-1]
                    ...
                    if len(dates) > 1:
                        ...
                        if isinstance(prev_data, dict):
                            prev_price = prev_data.get('close', prev_data.get('price', 0))
        ...
        def fetch():
            try:
                ...
                if price_data is not None and not price_data.empty:
                    ...
                    current_price = 0
                    for col in ['Close', 'close', 'Price', 'price', 'Adj Close', 'adj_close']:
                    ...
                    if current_price == 0:
                        for col in price_data.columns:
                            if pd.api.types.is_numeric_dtype(price_data[col]) and pd.notna(latest[col]):
                                current_price = float(latest[col])
                                break
        ...
        def fetch():
            try:
                ...
                if hist_data is not None and not hist_data.empty:
                    ...
                    if 'Date' not in hist_data.columns and hist_data.index.name != 'Date':
                    ...
                    return hist_data.sort_values('Date').reset_index(drop=True) if 'Date' in hist_data.columns else hist_data
        ...
        def fetch():
            try:
                ...
                try:
                    ...
                    results['income_statement'] = income_stmt if income_stmt is not None and not income_stmt.empty else None
                except Exception as e:
                ...
                try:
                    ...
                    results['balance_sheet'] = balance_sheet if balance_sheet is not None and not balance_sheet.empty else None
                except Exception as e:
                ...
                try:
                    ...
                    results['cash_flow'] = cash_flow if cash_flow is not None and not cash_flow.empty else None
                except Exception as e:
...
class ConsolidatedDataFeed:
    def __init__(self):
        ...
        self.source_priority = {
            'quote': [self.yfinance, self.fmp, self.finnhub, self.investiny, self.stockdex],
            'historical': [self.yfinance, self.fmp, self.investiny, self.stockdex],
            'company_info': [self.yfinance, self.fmp, self.finnhub, self.investiny, self.stockdex],
            ...
            try:
                if hasattr(adapter, 'get_historical'):
                    ...
                    if hist is not None and not hist.empty:
                        logger.info(f"Historical data for {symbol} from {adapter.source.value}")
                        return hist
            except Exception as e:
                logger.warning(f"Failed to get historical data from {adapter.source.value}: {e}")
                continue
    ...
    def get_company_info(self, symbol: str) -> Optional[CompanyInfo]:
        ...
        for adapter in self.source_priority['company_info']:
            try:
                ...
                logger.warning(f"Failed to get company info from {adapter.source.value}: {e}")
                continue
    ...
    def get_financial_statements(self, symbol: str) -> Dict[str, Optional[pd.DataFrame]]:
        ...
        if hasattr(self.stockdex, 'get_financial_statements') and self.stockdex.available:
            try:
                financials = self.stockdex.get_financial_statements(symbol)
                if financials and any(v is not None and not v.empty for v in financials.values() if hasattr(v, 'empty')):
                    logger.info(f"Financial statements for {symbol} from {self.stockdex.source.value}")
                    return financials
            except Exception as e:
...
if __name__ == "__main__":
    ...
    if quote:
        print(f"AAPL: ${quote.price} ({quote.change:+.2f}, {quote.change_percent:+.2f}%) from {quote.source}")
