        ...
        try:
            ...
            try:
                ...
                if income_stmt is not None and not income_stmt.empty:
                    ...
                    recent_eps = (net_income / shares_outstanding) if shares_outstanding > 0 else 0
                    
                    # Estimate next earnings date (typically quarterly)
                    last_earnings_date = income_stmt.columns[0]  # Most recent date
                    estimated_next_date = last_earnings_date + timedelta(days=90)  # ~3 months
                else:
                ...
                logger.warning(f"Could not fetch earnings data for {ticker}: {earnings_error}")
                recent_eps = 0
                estimated_next_date = datetime.now() + timedelta(days=30)
