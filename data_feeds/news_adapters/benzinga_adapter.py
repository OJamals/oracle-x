...
Fetches financial news and market analysis from Benzinga RSS feeds
"""
...
class BenzingaAdapter(BaseNewsAdapter):
    """Benzinga financial news adapter"""
    
    def __init__(self):
        # Benzinga News RSS feed (working feed from FeedSpot)
        super().__init__(
    ...
    def fetch_news_articles(self, symbol: str, limit: int = 20) -> List[Dict[str, Any]]:
        """
        Fetch Benzinga articles with additional filtering for financial relevance
        """
        # Get base articles from RSS
        articles = self._fetch_from_rss(symbol, limit)
        
        # Additional Benzinga-specific filtering
        filtered_articles = []
        for article in articles:
            if self._is_financial_article(article):
                filtered_articles.append(article)
                
        logger.info(f"Filtered {len(filtered_articles)} financial articles from {len(articles)} Benzinga articles for {symbol}")
    ...
    def _is_financial_article(self, article: Dict[str, Any]) -> bool:
        ...
        financial_keywords = [
            ...
            'premarket', 'aftermarket', 'options', 'calls', 'puts',
    ...
    def _is_relevant_to_symbol(self, article: Dict[str, Any], symbol: str) -> bool:
        """Enhanced symbol relevance check for Benzinga"""
        # Use base class logic first
        if super()._is_relevant_to_symbol(article, symbol):
            return True
            
        # Additional Benzinga-specific logic
        text = f"{article.get('title', '')} {article.get('description', '')}".lower()
        
        # Check for ticker format variations common on Benzinga
        symbol_variations = [
            f"{symbol.lower()}",
            f"${symbol.lower()}",
            f"({symbol.lower()})",
