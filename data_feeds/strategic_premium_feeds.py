...
class StrategicPremiumFeeds:
    ...
    def __init__(self):
        ...
        self.finnhub_key = os.environ.get("FINNHUB_API_KEY") or getattr(
            config_data, "finnhub_api_key", None
        )
        self.fmp_key = (
    ...
    def get_unique_finnhub_data(
        self, symbols: List[str], max_symbols: int = 5
    ) -> Dict[str, Any]:
        """
                ...
                if resp.status_code == 200:
                    ...
                    if data and len(data) > 0:
                        ...
                        buys = [
                            t
                            for t in data
                            if t.get("transactionType", "").lower()
                            in ["p-purchase", "m-exempt"]
                        ]
                        sells = [
                            t
                            for t in data
                            if t.get("transactionType", "").lower() in ["s-sale"]
                        ]
    ...
    def get_all_premium_data(
        ...
        """
        logger.info(
            f"Fetching premium data for {min(len(symbols), max_symbols)} symbols"
        )
...
def fetch_premium_unique_data(
    symbols: List[str], max_symbols: int = 5
) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "available": True,
            "finnhub": premium.get_unique_finnhub_data(
                selected, max_symbols=max_symbols
            ),
