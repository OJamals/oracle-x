...
try:
    ...
    logging.warning(
        "FinanceToolkit not available. Install with: pip install financetoolkit"
    )
...
try:
    ...
    logging.warning(
        "FinanceDatabase not available. Install with: pip install financedatabase"
    )
...
class FinanceToolkitAdapter:
    ...
    def __init__(self, cache, rate_limiter):
        ...
        self.api_key = (
            ...
            or (
                getattr(config_data, "financial_modeling_prep_api_key", None)
                if config_data
                else None
            )
        )
        if not self.api_key:
            logger.info(
                "No FMP API key found. FinanceToolkit will use free data sources with limitations"
            )
    ...
    def get_financial_ratios(
        self, symbols: Union[str, List[str]], period: str = "annual"
            ...
            try:
                ...
                for symbol in symbols:
                    ratios = FinancialRatios(
                        symbol=symbol, period=period, timestamp=datetime.now()
                    )
                        ...
                        for ratio_name, ratio_value in profitability.items():
                            if symbol.upper() in str(ratio_name).upper():
                                if "Gross Margin" in str(ratio_name):
                                    ratios.gross_margin = (
                                        float(ratio_value.iloc[-1])
                                        if not pd.isna(ratio_value.iloc[-1])
                                        else None
                                    )
                                    ...
                                    ratios.return_on_invested_capital = (
                                        float(ratio_value.iloc[-1])
                                        if not pd.isna(ratio_value.iloc[-1])
                                        else None
                                    )
                        ...
                        for ratio_name, ratio_value in valuation.items():
                            if symbol.upper() in str(ratio_name).upper():
                                if "Price-to-Earnings Ratio" in str(ratio_name):
                                    ...
                                    ratios.enterprise_value_to_ebitda = (
                                        float(ratio_value.iloc[-1])
                                        if not pd.isna(ratio_value.iloc[-1])
                                        else None
                                    )
    ...
    def get_historical_analysis(
        ...
        try:
            toolkit = self.create_toolkit(
                symbols, start_date, end_date, quarterly=False
            )
            if not toolkit:
                return None
...
class EnhancedFinanceDatabaseAdapter:
            ...
            if FINANCE_DATABASE_AVAILABLE:
                ...
                try:
                    ...
                    if hasattr(fd, "Cryptocurrencies"):
                        ...
                        logger.info(
                            "Cryptocurrencies not available in this FinanceDatabase version"
                        )
    ...
    def search_securities(
        self, query: Optional[str] = None, asset_class: Optional[str] = None, **filters
    ) -> Dict[str, List[SecurityInfo]]:
    ...
    def _search_equities(
        self, query: Optional[str] = None, **filters
    ) -> List[SecurityInfo]:
