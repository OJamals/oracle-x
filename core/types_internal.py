...
class MarketData(BaseModel):
    ...
    quality_score: float = Field(
        default=1.0, ge=0.0, le=1.0, description="Data quality score (0.0-1.0)"
    )
    ...
    def validate_symbol_format(cls, v):
        ...
        if not re.match(r"^[A-Z]+$", v):
            raise ValueError(
                f"Invalid symbol format: {v}. Must be uppercase letters only."
            )
        return v
...
class OptionContract(BaseModel):
    ...
    style: OptionStyle = Field(
        default=OptionStyle.AMERICAN, description="Exercise style"
    )
    bid: Optional[Decimal] = Field(
        None, ge=0, decimal_places=4, description="Bid price"
    )
    ask: Optional[Decimal] = Field(
        None, ge=0, decimal_places=4, description="Ask price"
    )
    last: Optional[Decimal] = Field(
        None, ge=0, decimal_places=4, description="Last trade price"
    )
    ...
    implied_volatility: Optional[Decimal] = Field(
        None, ge=0, le=5, decimal_places=4, description="Implied volatility (0-5.0)"
    )
    underlying_price: Optional[Decimal] = Field(
        None, gt=0, decimal_places=4, description="Underlying asset price"
    )
...
def _calculate_option_quality(data: OptionContract) -> float:
    ...
    if data.bid is not None and data.ask is not None:
        ...
        if spread <= 0:
            ...
            data.underlying_price is not None
            and spread > data.underlying_price * Decimal("0.1")
        ):
            score *= 0.7
...
class MarketDataExtended(MarketData):
    ...
    data_quality_score: float = Field(
        default=1.0, ge=0.0, le=1.0, description="Enhanced data quality score (0.0-1.0)"
    )
    source_reliability: float = Field(
        default=0.8,
        ge=0.0,
        le=1.0,
        description="Data source reliability rating (0.0-1.0)",
    )
    freshness_score: float = Field(
        default=1.0, ge=0.0, le=1.0, description="Time-based freshness metric (0.0-1.0)"
    )
    completeness_score: float = Field(
        default=1.0,
        ge=0.0,
        le=1.0,
        description="Field completeness assessment (0.0-1.0)",
    )
...
class OptionContractEnhanced(OptionContract):
    ...
    delta: Optional[Decimal] = Field(
        None, ge=-1.0, le=1.0, decimal_places=4, description="Delta Greek value"
    )
    gamma: Optional[Decimal] = Field(
        None, ge=0.0, le=1.0, decimal_places=4, description="Gamma Greek value"
    )
    theta: Optional[Decimal] = Field(
        None, le=0.0, decimal_places=4, description="Theta Greek value"
    )
    vega: Optional[Decimal] = Field(
        None, ge=0.0, decimal_places=4, description="Vega Greek value"
    )
    rho: Optional[Decimal] = Field(
        None, decimal_places=4, description="Rho Greek value"
    )
    iv_surface_position: Optional[Tuple[float, float]] = Field(
        None, description="Position in IV surface matrix (moneyness, days_to_expiry)"
    )
    liquidity_score: float = Field(
        default=50.0,
        ge=0.0,
        le=100.0,
        description="Combined volume/OI liquidity score (0-100)",
    )
    spread_ratio: Optional[float] = Field(
        None, ge=0.0, description="Bid-ask spread as percentage of underlying"
    )
    time_decay_risk: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Time decay risk assessment (0.0-1.0)"
    )
