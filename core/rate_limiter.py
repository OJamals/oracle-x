...
class RateLimiter:
    ...
    def set_circuit_breaker(self, source: DataSource, config: CircuitBreakerConfig) -> None:
        """
    ...
    def _token_bucket_strategy(self, key: str, config: RateLimitConfig) -> bool:
        ...
        self.cache.redis_client.expire(state_key, config.time_window * 2) if self.use_redis else None
    ...
    def record_failure(self, source: DataSource) -> None:
        ...
        with self._lock:
            ...
            if current_state == CircuitBreakerState.HALF_OPEN:
                ...
                logger.warning(f"Circuit re-opened for {source.name} after failure in half-open state")
                return
            ...
            if failure_count >= self.circuit_breakers[source].failure_threshold:
                ...
                self.cache.set_hash_field(key, 'state', CircuitBreakerState.OPEN.name)
                self.cache.set_hash_field(key, 'last_failure', time.time())
                logger.warning(f"Circuit opened for {source.name} after {failure_count} failures")
            else:
    ...
    def get_stats(self, source: DataSource) -> RateLimitStats:
        ...
        with self._lock:
            ...
            if stats.total_requests > 0:
                stats.current_rps = stats.allowed_requests / max(1, (time.time() - (stats.last_reset or time.time()).timestamp()))
