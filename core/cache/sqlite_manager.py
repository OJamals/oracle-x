...
class SQLiteCacheManager:
    ...
    def get_cached_data(self, key: str, ttl: Optional[int] = None) -> Optional[Any]:
        ...
        with self._lock:
            ...
            try:
                ...
                if ttl is not None:
                    conn.execute("UPDATE cache SET ttl = ? WHERE key = ?", (ttl, key))
            ...
            try:
                ...
                if isinstance(data, (MarketData, OptionContract)):
                    serialized_data = json.dumps(
                        data.dict(), default=self._json_serializer
                    )
                else:
    ...
    def get_keys(self, pattern: str = "%") -> List[str]:
        ...
        with self._lock:
            ...
            try:
                cursor = conn.execute(
                    "SELECT key FROM cache WHERE key LIKE ?", (pattern,)
                )
    ...
    def get_ttl(self, key: str) -> Optional[int]:
        ...
        with self._lock:
            ...
            try:
                cursor = conn.execute(
                    "SELECT ttl, created_at FROM cache WHERE key = ?", (key,)
                )
                row = cursor.fetchone()
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            try:
                ...
                if self.db_path != ":memory:":
                    stats["db_size_mb"] = Path(self.db_path).stat().st_size / (
                        1024 * 1024
                    )
                else:
...
def get_unified_cache_manager(
    prefer_redis: bool = True, **kwargs
) -> Union[SQLiteCacheManager, Any]:
    """
