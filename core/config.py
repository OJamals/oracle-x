...
class DatabaseConfig:
    ...
    def __post_init__(self):
        ...
        self.model_monitoring_db = os.environ.get('MODEL_MONITORING_DB_PATH', self.model_monitoring_db)
        self.prompt_optimization_db = os.environ.get('PROMPT_OPTIMIZATION_DB_PATH', self.prompt_optimization_db)
...
class ModelConfig:
    ...
    fallback_models: List[str] = field(default_factory=lambda: ["gpt-4o", "o4-mini", "o3-mini"])
...
class DataFeedConfig:
    ...
    def __post_init__(self):
        ...
        self.alphavantage_api_key = os.environ.get('ALPHAVANTAGE_API_KEY', self.alphavantage_api_key)
        self.twitter_bearer_token = os.environ.get('TWITTER_BEARER_TOKEN', self.twitter_bearer_token)
        self.reddit_client_id = os.environ.get('REDDIT_CLIENT_ID', self.reddit_client_id)
        self.reddit_client_secret = os.environ.get('REDDIT_CLIENT_SECRET', self.reddit_client_secret)
        self.iex_api_key = os.environ.get('IEX_API_KEY', self.iex_api_key)
        self.tradingeconomics_api_key = os.environ.get('TRADINGECONOMICS_API_KEY', self.tradingeconomics_api_key)
        ...
        self.google_news_api_key = os.environ.get('GOOGLE_NEWS_API_KEY', self.google_news_api_key)
...
class VectorDBConfig:
    ...
    def __post_init__(self):
        ...
        self.collection_name = os.environ.get('VECTOR_DB_COLLECTION', self.collection_name)
...
class OptimizationConfig:
    ...
    def __post_init__(self):
        """Override from environment variables"""
        self.enabled = os.environ.get('OPTIMIZATION_ENABLED', str(self.enabled)).lower() == 'true'
        self.population_size = int(os.environ.get('OPTIMIZATION_POPULATION_SIZE', self.population_size))
        self.mutation_rate = float(os.environ.get('OPTIMIZATION_MUTATION_RATE', self.mutation_rate))
...
class ConfigValidator:
    ...
    RECOMMENDED_CONFIGS = {
        'OPENAI_MODEL': {
            ...
            'default': 'gpt-4o',
            'validation': lambda x: x in ['gpt-4.1', 'gpt-4o', 'o4-mini', 'o3-mini', 'gpt-4o-mini'],
    ...
    def __init__(self, config: SystemConfig, level: ConfigValidationLevel = ConfigValidationLevel.PERMISSIVE):
        self.config = config
        self.level = level
    ...
    def validate(self) -> ConfigValidationResult:
        ...
        for key, spec in self.RECOMMENDED_CONFIGS.items():
            value = self._get_config_value(key)
            if not value:
                warnings.append(f"Recommended config missing: {key} - {spec['description']}")
            elif 'validation' in spec and not spec['validation'](value):
                warnings.append(f"Config may be invalid: {key} - {spec['error_msg']}")
        
        is_valid = len(errors) == 0 if self.level == ConfigValidationLevel.STRICT else len(missing_configs) == 0
