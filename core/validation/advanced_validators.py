...
class AdvancedValidators:
    ...
    def _initialize_rules(self) -> Dict[ValidationRuleType, List[ValidationRule]]:
        ...
        rules[ValidationRuleType.STATISTICAL] = [
            ValidationRule(
                ...
                enabled=self.validation_level in [ValidationLevel.STRICT, ValidationLevel.PARANOID]
            ),
            ValidationRule(
                rule_type=ValidationRuleType.STATISTICAL,
            ...
            for rule in rules:
                if rule.enabled:
                    try:
                        ...
                        result.errors.append(f"Validation error in {rule.name}: {str(e)}")
    ...
    def _calculate_quality_score(self, result: ValidationResult) -> float:
        ...
        # Penalize for errors
        if result.errors:
            score *= 0.5 ** min(len(result.errors), 5)  # Exponential decay for multiple errors
    ...
    def _validate_iv_range(self, data: Any, result: ValidationResult):
        if isinstance(data, OptionContract) and data.implied_volatility is not None:
            ...
            if iv < Decimal('0') or iv > Decimal('5.0'):
                result.errors.append(f"Implied volatility must be between 0 and 5.0: {iv}")
                result.is_valid = False
    ...
    def _validate_price_consistency(self, data: Any, result: ValidationResult):
        if isinstance(data, MarketData):
            if data.low > data.high:
                result.errors.append(f"Low price ({data.low}) cannot be greater than high price ({data.high})")
                result.is_valid = False
            
            if data.open > data.high or data.open < data.low:
                result.warnings.append(f"Open price ({data.open}) outside daily range ({data.low}-{data.high})")
            
            if data.close > data.high or data.close < data.low:
                result.warnings.append(f"Close price ({data.close}) outside daily range ({data.low}-{data.high})")
    
    def _validate_bid_ask_spread(self, data: Any, result: ValidationResult):
        if isinstance(data, OptionContract) and data.bid is not None and data.ask is not None:
            spread = data.ask - data.bid
            if spread <= Decimal('0'):
                result.errors.append(f"Bid-ask spread must be positive: bid={data.bid}, ask={data.ask}")
                result.is_valid = False
    ...
    def _validate_expiry_reasonableness(self, data: Any, result: ValidationResult):
        if isinstance(data, OptionContract):
            if data.expiry.year < 2000:
                ...
                result.is_valid = False
            elif data.expiry > datetime.now(data.expiry.tzinfo) + timedelta(days=365 * 3):
        ...
        if isinstance(data, MarketData) and self.historical_data.get(data.symbol):
            ...
            if len(historical_prices) >= 10:  # Need sufficient data
                ...
                if std > 0:
                    ...
                    if z_score > 3.0:
                        result.warnings.append(f"Price outlier detected (z-score: {z_score:.2f})")
        ...
        if isinstance(data, MarketData) and self.historical_data.get(data.symbol):
            ...
            if len(historical_volumes) >= 10:
                ...
                if data.volume > mean_volume * 5:
                    result.warnings.append(f"Volume anomaly detected: {data.volume} vs avg {mean_volume:.0f}")
    ...
    def _validate_source_reliability(self, data: Any, result: ValidationResult):
        if hasattr(data, 'source'):
            ...
            if reliability < 0.7:
                result.info.append(f"Data source reliability: {reliability:.1f} ({data.source.name})")


# Factory function for easy access
def get_advanced_validator(validation_level: ValidationLevel = ValidationLevel.STANDARD) -> AdvancedValidators:
    """Get an instance of AdvancedValidators with specified validation level."""
    return AdvancedValidators(validation_level)


# Utility functions for common validation tasks
def validate_market_data_advanced(data: MarketData, validation_level: ValidationLevel = ValidationLevel.STANDARD) -> ValidationResult:
...
def validate_option_contract_advanced(data: OptionContract, validation_level: ValidationLevel = ValidationLevel.STANDARD) -> ValidationResult:
