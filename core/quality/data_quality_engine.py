...
class DataQualityAssessor:
    ...
    def __init__(self, historical_window: int = 100, confidence_level: float = 0.95):
        ...
        self.source_performance: Dict[DataSource, List[bool]] = {}  # Track source success/failure
    ...
    def assess_option_quality(self, data: OptionContract) -> QualityAssessment:
        ...
        # Calculate individual metric scores
        assessment.quality_score.completeness = self._calculate_option_completeness(data)
    ...
    def _calculate_accuracy(self, data: MarketData) -> float:
        ...
        if data.symbol in self.historical_data and len(self.historical_data[data.symbol]) >= 10:
    ...
    def _calculate_timeliness(self, data: MarketData) -> float:
        ...
        now = datetime.now(data.timestamp.tzinfo) if data.timestamp.tzinfo else datetime.utcnow()
    ...
    def _calculate_option_completeness(self, data: OptionContract) -> float:
        ...
        optional_fields = ['bid', 'ask', 'last', 'volume', 'open_interest', 'implied_volatility', 'underlying_price']
        
        present_required = sum(1 for field in required_fields if getattr(data, field) is not None)
        present_optional = sum(1 for field in optional_fields if getattr(data, field) is not None)
        
        required_score = present_required / len(required_fields)
        optional_score = present_optional / len(optional_fields) if optional_fields else 1.0
    ...
    def _detect_anomalies(self, data: MarketData) -> List[AnomalyDetection]:
        ...
        if data.symbol in self.historical_data and len(self.historical_data[data.symbol]) >= 9:
            # Use the last 9 data points (excluding current data)
            historical_prices = [d.close for d in self.historical_data[data.symbol][-9:]]
            mean = statistics.mean(historical_prices)
            std = statistics.stdev(historical_prices) if len(historical_prices) > 1 else 0
        ...
        if data.symbol in self.historical_data and len(self.historical_data[data.symbol]) >= 9:
            # Use the last 9 data points (excluding current data)
            historical_volumes = [d.volume for d in self.historical_data[data.symbol][-9:]]
    ...
    def _detect_option_anomalies(self, data: OptionContract) -> List[AnomalyDetection]:
        ...
        if data.bid is not None and data.ask is not None and data.underlying_price is not None:
            spread_ratio = (data.ask - data.bid) / data.underlying_price
    ...
    def _generate_recommendations(self, assessment: QualityAssessment) -> List[str]:
        ...
        if score.completeness < 0.8:
            recommendations.append("Consider fetching missing data fields from alternative sources")
        
        if score.consistency < 0.7:
            recommendations.append("Verify data consistency with source provider")
        
        if score.timeliness < 0.6:
            recommendations.append("Consider using more recent data or real-time sources")
        
        if score.reliability < 0.7:
            recommendations.append("Consider using higher reliability data sources")
        
        if assessment.anomalies:
            recommendations.append("Review detected anomalies before using data for trading decisions")
        
        return recommendations
    
    def _generate_option_recommendations(self, assessment: QualityAssessment) -> List[str]:
        """Generate quality improvement recommendations for options."""
        recommendations = []
        score = assessment.quality_score
        
        if score.completeness < 0.7:
            recommendations.append("Fetch missing option data fields (bid, ask, IV, etc.)")
    ...
    def _update_historical_data(self, data: MarketData):
        ...
        if len(self.historical_data[data.symbol]) > self.historical_window:
            self.historical_data[data.symbol] = self.historical_data[data.symbol][-self.historical_window:]
    ...
    def _update_historical_options(self, data: OptionContract):
        ...
        if len(self.historical_options[underlying_symbol]) > self.historical_window:
            self.historical_options[underlying_symbol] = self.historical_options[underlying_symbol][-self.historical_window:]


# Factory function for easy access
def get_data_quality_assessor(historical_window: int = 100, confidence_level: float = 0.95) -> DataQualityAssessor:
