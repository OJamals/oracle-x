...
from utils.performance_monitor import get_performance_monitor, monitor_decorator, monitor_performance
...
def test_core_types():
    ...
    try:
        ...
        print(f"✓ MarketData validation passed: {validated_data.symbol} at {validated_data.close}")
    ...
    try:
        ...
        if result.symbol == 'AAPL' and result.close == Decimal('151.80'):
            ...
            if metrics['total_calls'] > 0:
                print(f"✓ Performance metrics collected: {metrics['total_calls']} calls")
            else:
