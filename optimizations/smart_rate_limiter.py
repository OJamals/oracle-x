...
class SmartRateLimiter:
        ...
        if minute_requests >= config.requests_per_minute:
            ...
            for timestamp in history:
                if timestamp > minute_ago:
                    ...
                    logger.debug(f"{source}: Rate limit approaching, waiting {wait_time:.1f}s")
                    return wait_time
            ...
            if hour_requests >= config.requests_per_hour:
                ...
                for timestamp in history:
                    if timestamp > hour_ago:
                        ...
                        logger.info(f"{source}: Hourly limit reached, waiting {wait_time:.1f}s")
                        return wait_time
    ...
    def get_stats(self, source: str) -> dict:
        ...
        return {
            ...
            'in_backoff': source in self.backoff_until and time.time() < self.backoff_until[source],
            'utilization_percent': (
...
async def test_rate_limiter():
    ...
    for i in range(15):
        ...
        if (i + 1) % 5 == 0:
            ...
            print(f"  Stats: {stats['requests_last_minute']}/{stats['limit_per_minute']} "
