...
def test_market_internals():
    ...
    try:
        ...
        if data and "breadth" in data and "vix" in data:
            ...
            print(
                f"   VIX: {data.get('vix', 'N/A')}, Sentiment: {data.get('market_sentiment', 'N/A')}"
            )
...
def test_finviz_breadth():
    ...
    try:
        ...
        if data and len(data) > 0:
            ...
            print(
                f"   Advancers: {data.get('advancers', 'N/A')}, Decliners: {data.get('decliners', 'N/A')}"
            )
...
def generate_report():
    ...
    # Save report to file
    with open(
        "/Users/omar/Documents/Projects/oracle-x/data_feeds_report.json", "w"
    ) as f:
