...
class TestDataQualityAssessor:
    ...
    def test_anomaly_detection_price_outlier(self):
        ...
        price_anomalies = [
            ...
            if a.anomaly_type == AnomalyType.PRICE_OUTLIER
        ]
    ...
    def test_anomaly_detection_volume_spike(self):
        ...
        volume_anomalies = [
            a
            for a in assessment.anomalies
            if a.anomaly_type == AnomalyType.VOLUME_SPIKE
        ]
    ...
    def test_anomaly_detection_spread_anomaly(self):
        ...
        spread_anomalies = [
            a
            for a in assessment.anomalies
            if a.anomaly_type == AnomalyType.SPREAD_ANOMALY
        ]
    ...
    def test_recommendations_generation(self):
        ...
        assert any(
            "recent" in rec.lower() or "real-time" in rec.lower()
            for rec in assessment.recommendations
        )
        assert any(
            "reliability" in rec.lower() or "source" in rec.lower()
            for rec in assessment.recommendations
        )
...
class TestQualityScoreWeights:
    ...
    def test_score_weighting(self):
        ...
        market_data = MarketData(
            ...
            - timedelta(hours=12),  # Moderately stale
            open=Decimal("150.00"),
            high=Decimal("155.00"),
            low=Decimal("149.50"),
            close=Decimal("154.50"),
