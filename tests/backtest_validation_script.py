...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("backtest_validation.log"), logging.StreamHandler()],
)
...
class BacktestValidator:
        ...
        try:
            ...
            if data and len(data) > 0:
                ...
                missing = [
                    ...
                    if ind not in data[sample_symbol].columns
                ]
                if not missing:
            ...
            if pit_data is not None:
                ...
                logger.info(
                    f"✓ Point-in-time data working: {len(pit_data)} days ending {pit_data.index[-1].date()}"
                )
    ...
    def validate_risk_management(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for i, test_case in enumerate(test_cases):
                signal = {"confidence": test_case["confidence"], "action": "buy"}
                price = test_case["price"]

                # Calculate position size
                position_size = self.engine._calculate_position_size(
                    signal, price, portfolio.total_value
                )

                expected_max = test_case["expected_max_size"]
                if abs(position_size - expected_max) <= 10:  # Allow small variance
                    logger.info(
                        f"✓ Position sizing test {i+1} passed: {position_size} shares"
                    )
                else:
                    logger.error(
                        f"✗ Position sizing test {i+1} failed: got {position_size}, expected ~{expected_max}"
                    )
            ...
            if abs(portfolio.max_drawdown - drawdown) < 0.001:
                ...
                logger.error(
                    f"✗ Drawdown calculation error: {portfolio.max_drawdown} vs {drawdown}"
                )
    ...
    def validate_performance_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if sharpe_diff < 0.1:
                logger.info(
                    f"✓ Sharpe ratio validation passed: {metrics.sharpe_ratio:.2f} vs {manual_sharpe:.2f}"
                )
                results["sharpe_ratio"] = True
            else:
                logger.error(
                    f"✗ Sharpe ratio validation failed: {metrics.sharpe_ratio:.2f} vs {manual_sharpe:.2f}"
                )
            ...
            if sortino_diff < 0.1:
                logger.info(
                    f"✓ Sortino ratio validation passed: {metrics.sortino_ratio:.2f} vs {manual_sortino:.2f}"
                )
                results["sortino_ratio"] = True
            else:
                logger.error(
                    f"✗ Sortino ratio validation failed: {metrics.sortino_ratio:.2f} vs {manual_sortino:.2f}"
                )

            # Validate win rate
            winning_trades = sum(1 for p in positions if not p.is_open and p.pnl > 0)
            expected_win_rate = winning_trades / len(
                [p for p in positions if not p.is_open]
            )
            if abs(metrics.win_rate - expected_win_rate) < 0.01:
                ...
                logger.error(
                    f"✗ Win rate validation failed: {metrics.win_rate:.1%} vs {expected_win_rate:.1%}"
                )
                ...
                try:
                    ...
                    try:
                        ...
                        if wf_results.get("periods"):
                            ...
                            logger.info(
                                f"✓ Walk-forward validation completed for {strategy_name}"
                            )
                            ...
                            if wf_performance:
                                ...
                                logger.info(
                                    f"  Total Return: {wf_performance.total_return:.2%}"
                                )
                                logger.info(
                                    f"  Sharpe Ratio: {wf_performance.sharpe_ratio:.2f}"
                                )
                                logger.info(
                                    f"  Max Drawdown: {wf_performance.max_drawdown:.2%}"
                                )
                                logger.info(
                                    f"  Win Rate: {wf_performance.win_rate:.2%}"
                                )
                                logger.info(
                                    f"  Total Trades: {wf_performance.total_trades}"
                                )
                        ...
                        logger.warning(
                            f"Walk-forward test failed for {strategy_name}: {e}"
                        )
        ...
        try:
            ...
            if performance:
                ...
                if performance.max_drawdown < 0.50:  # Less than 50% drawdown
                    ...
                    logger.warning(
                        f"⚠ High drawdown in stress test: {performance.max_drawdown:.1%}"
                    )
    ...
    def generate_validation_report(self) -> str:
        ...
        all_tests_passed = all(
            [
                ...
                self.validation_results.get("performance_metrics", {}).get(
                    "passed", False
                ),
                self.validation_results.get("comprehensive_backtest", {}).get(
                    "passed", False
                ),
            ]
        )
        ...
        report.append(
            f"OVERALL STATUS: {'✓ PRODUCTION READY' if all_tests_passed else '⚠ NEEDS ATTENTION'}"
        )
        report.append("")
                ...
                for key, value in test_results.items():
                    if isinstance(value, bool):
                        ...
                        for sub_key, sub_value in value.items():
                            if isinstance(sub_value, float) and (
                                "return" in sub_key
                                or "rate" in sub_key
                                or "ratio" in sub_key
                            ):
                                report.append(
                                    f"    {sub_key.replace('_', ' ').title()}: {sub_value:.2%}"
                                )
                            else:
                                report.append(
        ...
        if not all_tests_passed:
            ...
            if not self.validation_results.get("data_quality", {}).get("passed", False):
                report.append(
                    "  - Data quality issues detected - check data validation logs"
                )

            if not self.validation_results.get("risk_management", {}).get(
                "passed", False
            ):
                report.append("  - Risk management parameters may need adjustment")

            if not self.validation_results.get("performance_metrics", {}).get(
                "passed", False
            ):
                report.append("  - Performance metrics calculation needs verification")

            if not self.validation_results.get("comprehensive_backtest", {}).get(
                "passed", False
            ):
...
def main():
    ...
    try:
        ...
        logger.info(
            "Validation complete. Report saved to backtest_validation_report.txt"
        )
