...
try:
    from data_feeds.anomaly_detector import detect_price_volume_anomalies, _as_numeric_series
...
class TestAnomalyDetector(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.standard_series = pd.Series([100, 105, 95, 110, 300, 105, 98, 100, 350, 103])
        
        # DataFrame with volume column containing anomalies
        self.test_df = pd.DataFrame({
            'date': pd.date_range(start='2023-01-01', periods=10),
            'price': [150.0, 152.0, 151.5, 153.0, 155.0, 154.0, 153.5, 152.5, 153.0, 154.5],
            'volume': [1000000, 1200000, 950000, 1100000, 5000000, 1050000, 980000, 1000000, 4500000, 1030000]
        })
    ...
    def test_anomaly_detection_series(self):
        ...
        self.assertEqual(len(anomalies), 0)  # With threshold 3.0, no anomalies should be detected
    ...
    def test_anomaly_detection_edge_cases(self):
        ...
        nan_anomalies = detect_price_volume_anomalies(self.series_with_nans, threshold=1.5)
