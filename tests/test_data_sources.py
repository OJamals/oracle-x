...
def test_yfinance():
    ...
    try:
        ...
        try:
            ...
            results['recommendations'] = recommendations is not None and len(recommendations) > 0
        except:
    ...
    try:
        ...
        try:
            ...
            if results['historical_stocks']:
                logger.info(f"Retrieved {len(hist_data)} historical data points for AAPL")
        except Exception as e:
...
def test_finnhub():
    ...
    try:
        ...
        try:
            ...
            results['candles'] = bool(candles and 's' in candles and candles['s'] == 'ok')
        except:
...
def test_financial_modeling_prep():
    ...
    try:
        ...
        # Test historical data
        try:
            response = requests.get(f"{base_url}/historical-price-full/AAPL?from=2024-01-01&to=2024-01-31&apikey={api_key}")
            if response.status_code == 200:
                data = response.json()
        ...
        # Test income statement
        try:
            response = requests.get(f"{base_url}/income-statement/AAPL?period=annual&apikey={api_key}")
            if response.status_code == 200:
                data = response.json()
        ...
        # Test balance sheet
        try:
            response = requests.get(f"{base_url}/balance-sheet-statement/AAPL?period=annual&apikey={api_key}")
            if response.status_code == 200:
                data = response.json()
        ...
        # Test cash flow
        try:
            response = requests.get(f"{base_url}/cash-flow-statement/AAPL?period=annual&apikey={api_key}")
            if response.status_code == 200:
                data = response.json()
...
def run_all_tests():
    ...
    for source, results in all_results.items():
        ...
        if status == 'SUCCESS':
            ...
            successful_endpoints = sum(1 for k, v in results.items() 
                                     if k not in ['status', 'rate_limit'] and v is True)
            total_endpoints = len([k for k in results.keys() 
                                 if k not in ['status', 'rate_limit']])
