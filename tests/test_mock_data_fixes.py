...
def test_dark_pools():
    ...
    try:
        ...
        if result["dark_pools"]:
            ...
            print(
                f"   Sample: {sample['ticker']} - {sample['block_size']:,} shares @ ${sample['price']}"
            )
        return True
...
def test_earnings_calendar():
    ...
    try:
        ...
        if result:
            sample = result[0]
            print(
                f"   Sample: {sample['ticker']} - {sample['date']} (Est: ${sample['estimate']})"
            )
...
def test_options_flow():
    ...
    try:
        ...
        if result["unusual_sweeps"]:
            ...
            print(
                f"   Sample: {sample['ticker']} {sample['direction']} ${sample['strike']} - Vol: {sample['volume']:,}"
            )
        return True
    except Exception as e:
