...
def test_advanced_sentiment_aggregated_counts():
    ...
    if adv is None:
        ...
        logging.warning("Advanced sentiment returned None (network/limits) - skipping assertions")
        return
    ...
    for k, v in counts.items():
        assert isinstance(v, int) and v >= 0, f"Count for {k} should be non-negative int"
    ...
    if counts.get("total_unique"):
        assert counts["total_unique"] <= cap * 3, "total_unique exceeds global truncation bound"

    logging.info(
        "Advanced aggregated sentiment OK: score=%.3f conf=%.3f counts=%s",
        getattr(adv, "sentiment_score", 0.0), getattr(adv, "confidence", 0.0), counts
    )
...
def test_rss_adapter_registration():
    ...
    orch = DataFeedOrchestrator()
    rss_key = ("RSS", "rss_news")  # tuple key registered by orchestrator when RSS active
    adapter = orch.adapters.get(rss_key)  # type: ignore[index]
    if not adapter:
        logging.warning("RSS adapter not registered (feedparser missing or no feeds) - skipping")
        return
    # Validate include_all flag respected from env
    assert getattr(adapter, "include_all", False) is True, "RSS_INCLUDE_ALL env flag not reflected in adapter"

    # Light sentiment fetch (non-fatal if None)
    sd = adapter.get_sentiment("AAPL", limit=10)
    if sd is None:
        logging.warning("RSS sentiment returned None (no headlines / parsing issue) - skipping deeper assertions")
        return
    assert sd.sample_size is None or sd.sample_size <= 10, "Sample size should respect limit"
    if sd.raw_data:
