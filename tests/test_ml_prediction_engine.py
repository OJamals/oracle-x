        ...
        for symbol in symbols:
            for pred_type in [
                ...
                try:
                    ...
                    if result:
                        print(
                            f"✓ Fallback prediction for {symbol} {pred_type.value}: {result.prediction:.3f} (confidence: {result.confidence:.3f})"
                        )
                    else:
                        print(
                            f"✗ Failed to get fallback prediction for {symbol} {pred_type.value}"
                        )
...
def test_feature_engineering():
    ...
    try:
        ...
        if not features.empty:
            print(
                f"✓ Feature engineering successful: {len(features)} samples, {len(features.columns)} features"
            )
            print(
                f"Features: {list(features.columns)[:10]}..."
            )  # Show first 10 features
            print(
                f"Target columns: {[col for col in features.columns if col.startswith('target_')]}"
            )
            return True
        else:
...
def test_ml_models():
    ...
    try:
        ...
        if not available_models:
            print(
                "No ML models available - this is expected if libraries not installed"
            )
            return True
        ...
        for model_type in available_models:
            for pred_type, y in [
                (PredictionType.PRICE_DIRECTION, y_class),
                ...
                try:
                    ...
                    print(
                        f"✓ Feature importance (top 3): {dict(list(importance.items())[:3])}"
                    )
