...
class OptimizationTestSuite:
    ...
    def test_ticker_validator_cache(self):
        ...
        if second_call < first_call * 0.5:
            return (
                True,
                f"Cache working (1st: {first_call:.3f}s, 2nd: {second_call:.3f}s)",
            )
        else:
    ...
    def test_smart_rate_limiter_basic(self):
        ...
        async def test_throttling():
            ...
            if times[0] < 0.1 and times[1] > 0:
                return (
                    True,
                    f"Throttling works (delays: {times[1]:.2f}s, {times[2]:.2f}s)",
                )
            else:
    ...
    def test_market_internals_caching(self):
        ...
        if second_call < first_call * 0.3 and data1 == data2:
            return (
                True,
                f"Caching works (1st: {first_call:.2f}s, 2nd: {second_call:.3f}s)",
            )
        else:
    ...
    def test_async_market_internals(self):
        ...
        async def test_fetch():
            ...
            if (
                data
                and "market_internals" in str(data).lower()
                or "indices" in str(data).lower()
            ):
    ...
    def test_async_parallel_execution(self):
        ...
        async def test_parallel():
            ...
            await asyncio.gather(
                fetch_market_internals_async(), fetch_finviz_breadth_async()
            )
            ...
            if speedup >= 1.0:
                return (
                    True,
                    f"Parallel {speedup:.1f}x faster ({parallel_time:.2f}s vs {sequential_time:.2f}s)",
                )
            else:
    ...
    def test_async_signal_orchestrator(self):
        ...
        async def test_orchestrator():
            ...
            signals = await get_signals_from_scrapers_async(
                "Test prompt", chart_image_b64=None, enable_premium=False
            )
            duration = time.time() - start
    ...
    def test_full_phase2_integration(self):
        ...
        async def test_integration():
            signals = await get_signals_from_scrapers_async(
                "Test integration", chart_image_b64=None, enable_premium=False
            )
            ...
            if present >= 2:
                return (
                    True,
                    f"Phase 2 integration working ({present}/{len(sources)} sources)",
                )
            else:
    ...
    def print_summary(self):
        ...
        if failed == 0:
            ...
            print(
                f"{Colors.OKGREEN}Phase 1 and Phase 2 optimizations are working correctly!{Colors.ENDC}\n"
            )
            return 0
        else:
...
def main():
    ...
    parser.add_argument(
        "--phase", "-p", type=int, choices=[1, 2], help="Run specific phase tests only"
    )
