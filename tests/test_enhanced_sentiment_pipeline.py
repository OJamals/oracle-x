...
def test_enhanced_sentiment_pipeline():
    ...
    try:
        ...
        from data_feeds.enhanced_sentiment_pipeline import (
            get_enhanced_sentiment_pipeline,
        )
        ...
        for symbol in test_symbols:
            ...
            start_time = time.time()
            orchestrator_result = get_enhanced_sentiment_analysis(
                symbol, include_reddit=False
            )
            orchestrator_time = time.time() - start_time

            print(f"   ⏱️  Orchestrator Time: {orchestrator_time:.2f}s")
            print(
                f"   ✅ Orchestrator Result: {orchestrator_result.get('overall_sentiment', 'error')}"
            )
    ...
    for adapter_name, adapter_import in news_adapters:
        try:
            ...
            if result:
                ...
                print(
                    f"   ⚠️  No {adapter_name} sentiment data (possibly no recent news)"
                )
...
def benchmark_performance():
    ...
    try:
        ...
        from data_feeds.data_feed_orchestrator import get_sentiment_data
        ...
        for symbol in test_symbols:
            ...
            print(f"   {symbol}: {processing_time:.2f}s " f"(sources: {len(result)})")
        ...
        improvement = (
            ((avg_enhanced_time - avg_original_time) / avg_original_time) * 100
