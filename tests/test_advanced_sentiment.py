...
def test_individual_models():
    ...
    test_cases = [
        (
            "AAPL",
            "Apple earnings beat expectations! Stock is going to the moon! 🚀",
            "bullish",
        ),
        (
            "TSLA",
            "Tesla is way overvalued. This bubble is about to crash hard.",
            "bearish",
        ),
        (
            "SPY",
            "Market is consolidating in this range. Waiting for breakout.",
            "neutral",
        ),
        (
            "NVDA",
            "NVIDIA guidance raised again. AI momentum continues strong.",
            "bullish",
        ),
        (
            "META",
            "Meta missing user growth targets. Bearish for next quarter.",
            "bearish",
        ),
    ]
...
def test_batch_analysis():
    ...
    for result in results:
        print(
            f"  {result.symbol}: {result.ensemble_score:.3f} (conf: {result.confidence:.3f})"
        )
...
def test_symbol_aggregation():
    ...
    summary = analyze_symbol_sentiment(
        "AAPL", aapl_texts, ["news", "research", "analyst", "supply", "flow"]
    )
    ...
    print(
        f"  Bullish: {summary.bullish_mentions}, Bearish: {summary.bearish_mentions}, Neutral: {summary.neutral_mentions}"
    )
...
def test_financial_lexicon():
    ...
    for i, result in enumerate(results):
        print(
            f"  Text {i+1}: Ensemble: {result.ensemble_score:.3f}, Confidence: {result.confidence:.3f}"
        )
...
def test_model_performance_tracking():
    ...
    for model, perf in engine.model_performance.items():
        print(
            f"  {model}: Accuracy: {perf['accuracy']:.2f}, Weight: {perf['weight']:.2f}"
        )
