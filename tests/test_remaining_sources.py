    ...
    try:
        ...
        try:
            ...
            if apple_results:
                ...
                if results["historical_stocks"]:
                    logger.info(
                        f"Retrieved {len(hist_data)} days of historical data for Apple"
                    )
        ...
        try:
            if apple_results:
                ...
                results["historical_3months"] = bool(
                    month3_data and len(month3_data) > 0
                )
        ...
        try:
            ...
            if crypto_results:
                ...
                results["historical_crypto"] = bool(
                    crypto_hist and len(crypto_hist) > 0
                )
            else:
...
def test_quantsumore_comprehensive():
    ...
    try:
        ...
        try:
            ...
            results["treasury_historical"] = bool(
                hist_treasury and len(hist_treasury) > 0
            )
        ...
        try:
            ...
            results["technical_rsi"] = bool(
                rsi_values is not None and len(rsi_values) > 0
            )

            # Test SMA calculation
            sma_values = SMA(prices, period=20)
            results["technical_sma"] = bool(
                sma_values is not None and len(sma_values) > 0
            )

            # Test EMA calculation
            ema_values = EMA(prices, period=20)
            results["technical_ema"] = bool(
                ema_values is not None and len(ema_values) > 0
            )

            if results["technical_rsi"]:
                logger.info(
                    f"RSI calculation successful, latest value: {rsi_values.iloc[-1]:.2f}"
                )
        ...
        try:
            ...
            logger.info(
                "Fundamental analysis unavailable due to Yahoo Finance disruption"
            )
    ...
    try:
        ...
        try:
            ...
            if results["company_info"]:
                ...
                logger.info(
                    f"Company info keys: {list(company_info.keys()) if isinstance(company_info, dict) else 'Not a dict'}"
                )
    ...
    for source_name, results in all_results.items():
        ...
        if "SUCCESS" in status:
            ...
            if successful > 0:
                ...
                print(
                    f"   Best for: {results.get('data_coverage', 'Various data types')}"
                )
