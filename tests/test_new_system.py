...
from data_feeds.oracle_data_interface import (
    get_oracle_provider,
    get_signals_from_scrapers_v2,
)
...
def test_oracle_interface():
    ...
    if "error" not in internals:
        print(
            f"✓ SPY: ${internals.get('spy_price', 'N/A')} ({internals.get('spy_change', 'N/A')}%)"
        )
...
def test_backward_compatibility():
    ...
    try:
        ...
        print(
            f"  Overall Quality Score: {signals['data_quality']['overall_score']:.1f}%"
        )
