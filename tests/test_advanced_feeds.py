...
def test_concurrent_performance():
    ...
    print(
        f"  Results: {len([r for r in sequential_results if r[1] > 0])}/{len(tickers)} successful"
    )
    ...
    print(
        f"  Results: {len([r for r in concurrent_results if r[1] > 0])}/{len(tickers)} successful"
    )
...
def test_data_quality():
    ...
    for ticker in test_tickers:
        ...
        negative_prices = any(
            (df[col] < 0).any() for col in price_cols if col in df.columns
        )
        ...
        if "Volume" in df.columns:
            ...
            if zero_volume_days < len(df) * 0.1:  # Less than 10% zero volume days
                print(
                    f"  ✅ Reasonable volume data ({zero_volume_days} zero-volume days)"
                )
                checks.append(True)
            else:
...
def test_rate_limiting():
    ...
    cache_working = (
        results[1] > 0 and total_time < 30
    )  # Should complete in reasonable time
    print(
        f"  Caching effectiveness: {'✅ Working' if cache_working else '❌ Not working'}"
    )
...
def test_memory_usage():
    ...
    for i, ticker in enumerate(tickers):
        ...
        if (i + 1) % 5 == 0:  # Check every 5 calls
            ...
            print(
                f"  After {i+1} calls: {current_memory:.1f} MB (+{memory_increase:.1f} MB)"
            )
...
def test_orchestrator_integration():
    ...
    try:
        ...
        signals_valid = (
            "timestamp" in signals
            and "data_source" in signals
            and signals.get("signals_count", 0) >= 0
        )
        ...
        print(
            f"  Fallback manager active: {health.get('fallback_manager_active', False)}"
        )
        ...
        health_valid = (
            "timestamp" in health
            and "redis_connected" in health
            and "adapters_loaded" in health
            and health.get("adapters_loaded", 0) > 0
        )
...
def main():
    ...
    print("\n" + "=" * 80)
    print(
        f"📊 ADVANCED TEST SUMMARY: {passed}/{total} tests passed ({passed/total*100:.1f}%)"
    )
