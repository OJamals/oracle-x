    ...
    if not data or not data.get("ok"):
        if "skipped" in str(data.get("error", "")).lower():
            return ValidationResult(
                True, "Skipped due to missing API key", {"reason": "skipped"}
            )
        return ValidationResult(
            False, f"Quote fetch failed: {data.get('error', 'Unknown error')}"
        )
...
def validate_market_data(data, symbol):
    ...
    if not data or not data.get("ok"):
        ...
        return ValidationResult(
            False, f"Market data fetch failed: {data.get('error', 'Unknown error')}"
        )
    ...
    if last_row:
        try:
            if "Close" in last_row and last_row["Close"] is not None:
                ...
                if close_price <= 0:
                    return ValidationResult(
                        False, f"Invalid close price: {close_price}"
                    )
        except (TypeError, ValueError):
            return ValidationResult(
                False, f"Invalid close price format: {last_row.get('Close')}"
            )
        ...
        try:
            ...
            return ValidationResult(
                False, f"Invalid volume format: {last_row.get('Volume')}"
            )
...
def validate_company_info(data, symbol):
    ...
    if not data or not data.get("ok"):
        ...
        return ValidationResult(
            False, f"Company info fetch failed: {data.get('error', 'Unknown error')}"
        )
...
def validate_news(data, symbol):
    ...
    if not data or not data.get("ok"):
        ...
        return ValidationResult(
            False, f"News fetch failed: {data.get('error', 'Unknown error')}"
        )
...
def validate_multiple_quotes(data, symbol):
    ...
    if not data or not data.get("ok"):
        ...
        return ValidationResult(
            False, f"Multiple quotes fetch failed: {data.get('error', 'Unknown error')}"
        )
    ...
    for sym, quote in quotes.items():
        ...
        if (
            not quote_result.passed
            and quote_result.reason != "Skipped due to missing API key"
        ):
            return ValidationResult(
                False, f"Invalid quote for {sym}: {quote_result.reason}"
            )
...
def validate_financial_statements(data, symbol):
    ...
    if not data or not data.get("ok"):
        ...
        return ValidationResult(
            False,
            f"Financial statements fetch failed: {data.get('error', 'Unknown error')}",
        )
...
def validate_sentiment(data, symbol):
    ...
    if not data.get("ok"):
        if (
            "skipped" in str(data.get("error", "")).lower()
            or "skipped" in str(data.get("note", "")).lower()
        ):
            return ValidationResult(
                True,
                "Skipped due to missing API key or no sentiment available",
                {"reason": "skipped"},
            )
        # For invalid symbols, empty results are expected
        if symbol == "INVALID1234":
            return ValidationResult(True, "Expected empty result for invalid symbol")
        return ValidationResult(
            False, f"Sentiment fetch failed: {data.get('error', 'Unknown error')}"
        )
    ...
    if not sentiment_data and symbol != "INVALID1234":
        return ValidationResult(
            True, "No sentiment data available", {"reason": "no_data"}
        )
    ...
    for source, sd in sentiment_data.items():
        if not isinstance(sd, dict):
            return ValidationResult(
                False, f"Invalid sentiment format for source {source}"
            )
        ...
        if score is not None:
            try:
                ...
                if not -1 <= score_val <= 1:
                    return ValidationResult(
                        False, f"Invalid sentiment score for {source}: {score_val}"
                    )
            except (TypeError, ValueError):
                return ValidationResult(
                    False, f"Invalid sentiment score format for {source}: {score}"
                )
...
def validate_advanced_sentiment(data, symbol):
    ...
    if not data.get("ok"):
        if (
            ...
            return ValidationResult(
                True,
                "Skipped due to missing API key or no texts available",
                {"reason": "skipped"},
            )
        # For invalid symbols, empty results are expected
        if symbol == "INVALID1234":
        ...
        return ValidationResult(
            False,
            f"Advanced sentiment fetch failed: {data.get('error', 'Unknown error')}",
        )
    ...
    if data.get("score") is not None:
        ...
        try:
            ...
            return ValidationResult(
                False, f"Invalid sentiment score format: {data.get('score')}"
            )
        ...
        try:
            ...
            return ValidationResult(
                False, f"Invalid confidence format: {data.get('confidence')}"
            )
...
def validate_market_breadth(data, symbol):
    ...
    if not data or not data.get("ok"):
        if (
            ...
            return ValidationResult(
                True,
                "Skipped due to unavailable adapter or missing key",
                {"reason": "skipped"},
            )
        return ValidationResult(
            False, f"Market breadth fetch failed: {data.get('error', 'Unknown error')}"
        )
    ...
    if put_call_ratio is not None:
        try:
            ...
            return ValidationResult(
                False, f"Invalid put_call_ratio format: {put_call_ratio}"
            )
...
def validate_sector_performance(data, symbol):
    ...
    if not data.get("ok"):
        ...
        return ValidationResult(
            False,
            f"Sector performance fetch failed: {data.get('error', 'Unknown error')}",
        )
            ...
            if field in row:
                ...
                if value is not None:
                    try:
                        ...
                        return ValidationResult(
                            False, f"Invalid {field} format in row {i}: {value}"
                        )
...
@pytest.mark.parametrize(
    "test_case",
    [tc for tc in TEST_CASES if "network" in tc["markers"]],
    ids=lambda x: x["name"],
)
...
@pytest.mark.parametrize(
    "test_case",
    [tc for tc in TEST_CASES if "slow" in tc["markers"]],
    ids=lambda x: x["name"],
)
...
@pytest.mark.parametrize(
    "test_case",
    [tc for tc in TEST_CASES if "optional" in tc["markers"]],
    ids=lambda x: x["name"],
)
    ...
    if "## Automated Test Results" in content:
        ...
        for line in lines:
            if line.strip() == "## Automated Test Results":
                ...
                in_automated_section
                and line.startswith("#")
                and not line.startswith("## Automated Test Results")
            ):
                in_automated_section = False
...
def test_cli_flags():
    ...
    if not stdout.strip():
        stdout, stderr, code = run_cli_command(
            ["python", "tests/cli_validate.py", "--json", "quote", "--symbol", "AAPL"]
        )
