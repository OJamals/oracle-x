...
def test_complete_pipeline_integration():
    ...
    # Summary
    successful_modes = [
        mode for mode, result in results.items() if result["status"] == "success"
    ]
...
def test_data_consistency():
    ...
    if len(df1) == len(df2):
        ...
        if not df1.empty and not df2.empty:
            price_diff = (
                abs(df1["Close"].iloc[-1] - df2["Close"].iloc[-1])
                / df1["Close"].iloc[-1]
            )
            if price_diff < 0.01:  # Less than 1% difference
                ...
                print(
                    f"  ⚠️  Price difference: {price_diff:.3%} (acceptable for live data)"
                )
                return True
                ...
                if isinstance(value, (int, float)):
                    if value == 100.0 or value == 1000000:  # Common synthetic values
                        synthetic_indicators.append(
                            f"Suspicious value at {new_path}: {value}"
                        )
