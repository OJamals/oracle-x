...
print(
    f'  - Price range: ${aapl_data["close"].min():.2f} - ${aapl_data["close"].max():.2f}'
)
...
class AdvancedSentimentStrategy(TradingStrategy):
    ...
    def generate_signal(self, symbol, data, sentiment_data=None):
        ...
        # Price momentum
        price_change = (
            (current_price - data.iloc[-5]["close"]) / data.iloc[-5]["close"]
            if len(data) >= 5
            else 0
        )
        ...
        # Combined signal
        combined_score = (tech_score * self.tech_weight) + (
...
for i in range(10, len(backtest_data)):  # Start after we have enough data
    ...
    if signal == 1 and current_position == 0:  # Buy
        shares = int(
            (capital * config.max_position_size) // data_slice.iloc[-1]["close"]
        )
        if shares > 0:
            ...
            capital -= cost
            positions.append(
                f'BUY {shares} shares at ${data_slice.iloc[-1]["close"]:.2f}'
            )
    elif signal == -1 and current_position > 0:  # Sell
        proceeds = (
            current_position
            * data_slice.iloc[-1]["close"]
            * (1 - config.commission_pct)
        )
        capital += proceeds
        positions.append(
            f'SELL {current_position} shares at ${data_slice.iloc[-1]["close"]:.2f}'
        )
        current_position = 0
...
try:
    ...
    if sentiment_data and "reddit" in sentiment_data:
        ...
        test_signal_real = strategy.generate_signal(
            "AAPL", aapl_data, {"reddit_sentiment": reddit_info.sentiment_score}
        )
