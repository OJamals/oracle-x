...
def test_quality_improvement():
    ...
    try:
        ...
        for symbol in test_symbols:
            ...
            print(
                f"     Sentiment: {result.get('overall_sentiment', 0):.3f} ({result.get('trending_direction', 'unknown')})"
            )
        ...
        # Save to file
        results_file = (
            f"sentiment_quality_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
def test_individual_components():
    ...
    try:
        ...
        test_cases = [
            (
                "AAPL beats earnings expectations with strong iPhone sales",
                "AAPL",
                "earnings",
            ),
            (
                "TSLA faces production delays and supply chain issues",
                "TSLA",
                "production",
            ),
...
def main():
    ...
    if quality_results:
        ...
        if quality_results["average_quality"] > 37.2:
            print(
                f"✅ SUCCESS: Quality score improved from 37.2 to {quality_results['average_quality']:.1f}"
            )
        else:
            print(
                f"⚠️  NEEDS WORK: Quality score below target (current: {quality_results['average_quality']:.1f})"
            )
