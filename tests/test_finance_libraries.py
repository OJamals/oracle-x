    ...
    try:
        ...
        try:
            ...
            if "Technology" in etf_categories:
                ...
                print(
                    "Note: Using 'Communication Services' as Technology category not available"
                )
        ...
        search_results = equities.search(name="Apple", country="United States")
        ...
        try:
            ...
            try:
                ...
                if roe is not None and not roe.empty:
                    print(
                        f"✅ Return on Equity calculated for {len(roe.columns)} companies"
                    )
                ...
                if current_ratio is not None and not current_ratio.empty:
                    print(
                        f"✅ Current Ratio calculated for {len(current_ratio.columns)} companies"
                    )
        ...
        try:
            ...
            try:
                ...
                if income_stmt is not None and not income_stmt.empty:
                    print(
                        f"✅ Income statements retrieved for {len(income_stmt.columns)} companies"
                    )

                balance_sheet = toolkit.get_balance_sheet_statement()
                if balance_sheet is not None and not balance_sheet.empty:
                    print(
                        f"✅ Balance sheets retrieved for {len(balance_sheet.columns)} companies"
                    )
            ...
            try:
                ...
                if ratios is not None and not ratios.empty:
                    print(
                        f"✅ Profitability ratios calculated for {len(ratios.columns)} companies"
                    )
                        ...
                        if historical is not None and not historical.empty:
                            try:
                                ...
                                symbol_cols = [
                                    ...
                                    if symbol in str(col) and "Close" in str(col)
                                ]
                                if symbol_cols:
...
def test_premium_api_endpoints():
    ...
    try:
        ...
        try:
            ...
            print(
                "Note: Quarterly data testing skipped - parameter not supported in current version"
            )
        ...
        try:
            ...
            if valuation_ratios is not None and not valuation_ratios.empty:
                ...
                market_cap_rows = [
                    idx for idx in valuation_ratios.index if "Market Cap" in str(idx)
                ]
                if market_cap_rows:
                ...
                ev_rows = [
                    idx
                    for idx in valuation_ratios.index
                    if "Enterprise Value" in str(idx)
                ]
                if ev_rows:
        ...
        try:
                ...
                try:
                    ...
                    if ratios is not None and not ratios.empty:
                        ...
                        if tickers[0] in ratios.columns:
                            latest_ratios = (
                                ratios[tickers[0]].dropna().iloc[-5:]
                            )  # Last 5 ratios
                            print(
                                f"  Latest {category_name.lower()} ratios for {tickers[0]}:"
                            )
            ...
            if all_ratios_data:
                total_ratios = sum(
                    len(ratios.index) for ratios in all_ratios_data.values()
                )
                print(
        ...
        try:
            ...
            if roe_ratios is not None and not roe_ratios.empty:
                ...
                for ticker in tickers:
                    ticker_cols = [
                        col for col in roe_ratios.columns if ticker in str(col)
                    ]
                    if ticker_cols:
            ...
            if debt_equity is not None and not debt_equity.empty:
                ...
                for ticker in tickers:
                    ticker_cols = [
                        col for col in debt_equity.columns if ticker in str(col)
                    ]
                    if ticker_cols:
                        latest_de = debt_equity[ticker_cols[0]].dropna()
                        if not latest_de.empty:
                            print(
                                f"  {ticker} Latest D/E Ratio: {latest_de.iloc[-1]:.2f}"
                            )
            ...
            if current_ratios is not None and not current_ratios.empty:
                ...
                for ticker in tickers:
                    ticker_cols = [
                        col for col in current_ratios.columns if ticker in str(col)
                    ]
                    if ticker_cols:
                        latest_cr = current_ratios[ticker_cols[0]].dropna()
                        if not latest_cr.empty:
                            print(
                                f"  {ticker} Latest Current Ratio: {latest_cr.iloc[-1]:.2f}"
                            )
