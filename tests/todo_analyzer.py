...
class TodoAnalyzer:
    ...
    def _classify_priority(self, description: str, file_path: str) -> TodoPriority:
        ...
        for priority, keywords in self.PRIORITY_KEYWORDS.items():
            if any(
                keyword in description_lower or keyword in file_path_lower
    ...
    def _estimate_effort(self, description: str) -> str:
        ...
        if any(
            ...
            for word in ["implement", "integrate", "redesign", "refactor", "complete"]
        ):
            return "L"
        ...
        if any(
            word in description_lower
            for word in ["rename", "comment", "document", "fix typo", "update"]
        ):
            return "S"
    ...
    def generate_report(self) -> str:
        ...
        for priority in TodoPriority:
            ...
            if priority_todos:
                report.append(
                    f"## {priority.name} Priority Items ({len(priority_todos)})"
                )
                report.append("")

                for todo in sorted(
                    priority_todos, key=lambda x: (x.category, x.file_path)
                ):
        ...
        for priority in TodoPriority:
            ...
            for effort in ["S", "M", "L"]:
                if effort_groups[effort]:
                    ...
                    plan.append(
                        f"### {effort_name} Effort ({len(effort_groups[effort])} items)"
                    )

                    for todo in effort_groups[effort]:
                        plan.append(
                            f"- [ ] **{todo.file_path}:{todo.line_number}** - {todo.description}"
                        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze TODO/FIXME items in ORACLE-X codebase"
    )
    ...
    parser.add_argument(
        "--action-plan", action="store_true", help="Generate action plan"
    )
    ...
    if args.summary or (not args.report and not args.action_plan):
        ...
        if todos:
            ...
            for todo in todos:
                priority_counts[todo.priority] = (
                    priority_counts.get(todo.priority, 0) + 1
                )
                category_counts[todo.category] = (
                    category_counts.get(todo.category, 0) + 1
                )
