...
class NewsAdapterTester:
    ...
    def test_single_adapter(
        self, adapter_name: str, adapter: BaseNewsAdapter, symbol: str, limit: int = 10
    ) -> NewsAdapterTestResult:
        ...
        try:
            ...
            sentiment_available = (
                hasattr(adapter, "sentiment_engine")
                and adapter.sentiment_engine is not None
            )
    ...
    def _calculate_quality_score(
        self, articles: List[Dict[str, Any]], adapter_name: str
    ) -> float:
    ...
    def generate_test_report(self, results: List[NewsAdapterTestResult]) -> str:
        ...
        if failed:
            ...
            for result in failed:
                report_lines.append(
                    f"  - {result.adapter_name}: {result.error_message}"
                )
            report_lines.append("")
        ...
        if successful:
            ...
            for result in successful[:3]:
                report_lines.append(
                    f"  - {result.adapter_name}: {result.quality_score:.1f} quality, {result.response_time:.2f}s"
                )
...
def main():
    ...
    if success_rate >= 0.5:
        print(
            f"✅ Test Suite PASSED: {len(successful)}/{len(results)} adapters working"
        )
        return 0
    else:
        print(
            f"❌ Test Suite FAILED: Only {len(successful)}/{len(results)} adapters working"
        )
        return 1
