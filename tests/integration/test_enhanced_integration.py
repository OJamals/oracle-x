...
def test_enhanced_finance_database():
    ...
    try:
        ...
        results = adapter.search_securities(
            asset_class="equity", country="United States", sector="Technology"
        )
        ...
        if "equities" in results and results["equities"]:
            ...
            for i, security in enumerate(results["equities"][:3]):
                print(
                    f"   {i+1}. {security.symbol}: {security.name} ({security.sector})"
                )
        else:
        ...
        etf_results = adapter.search_securities(
            asset_class="etf", category_group="Equities"  # Using correct category
        )
...
def test_finance_toolkit_integration():
    ...
    try:
        ...
        symbols = ["AAPL", "MSFT"]
        toolkit = adapter.create_toolkit(
        ...
        historical = adapter.get_historical_analysis(
            symbols, start_date="2023-01-01", end_date="2023-12-31"
        )
        ...
        if historical is not None and not historical.empty:
            ...
            cols = [col for col in historical.columns if isinstance(col, tuple)][
                :10
            ]  # Show first 10
            for col in cols:
                print(f"   {col}")
        ...
        if ratios_dict and "AAPL" in ratios_dict:
            ...
            print(
                "⚠️  No financial ratios calculated (may need API key for full functionality)"
            )
...
def test_combined_workflow():
    ...
    try:
        ...
        # Get historical data
        historical = ft_adapter.get_historical_analysis(
            symbols, start_date="2023-01-01"
        )
        if historical is not None and not historical.empty:
            print(f"✅ Retrieved historical data: {historical.shape}")
                ...
                try:
                    ...
                    if close_col in historical.columns:
                        ...
                        if not prices.empty:
                            ...
                            volatility = (
                                prices.pct_change().std() * (252**0.5) * 100
                            )  # Annualized
