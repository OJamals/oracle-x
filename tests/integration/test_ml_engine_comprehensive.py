...
class MLEngineTestSuite:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.ml_engine = create_prediction_engine(
                self.data_orchestrator, self.sentiment_engine
            )
    ...
    def test_engine_initialization(self):
        ...
        try:
            ...
            assert hasattr(
                self.ml_engine, "data_orchestrator"
        ...
        try:
            ...
            for pred_type in prediction_types:
                for horizon in horizons:
                    logger.info(
                        f"🔮 Testing {pred_type.value} prediction for {horizon} days"
                    )
                    ...
                    if result:
                        ...
                        logger.warning(
                            f"⚠️  No prediction for {pred_type.value}_{horizon}d"
                        )

            # At least some predictions should work
            assert successful_predictions > 0, "No predictions were successful"
            logger.info(
                f"📊 {successful_predictions}/{len(prediction_types) * len(horizons)} predictions successful"
            )
    ...
    def test_training_process(self):
        ...
        try:
            ...
            if training_results:
                logger.info(
                    f"📚 Training completed with {len(training_results)} result sets"
                )
                for target, results in training_results.items():
                    logger.info(
                        f"📚 {target}: {len(results) if isinstance(results, dict) else 'completed'} models"
                    )
            else:
                logger.info(
                    "📚 Training used fallback method (ML engine not fully available)"
                )
        ...
        try:
            ...
            if performance:
                ...
                for model_key, metrics in performance.items():
                    ...
                    logger.info(
                        f"   - Predictions: {metrics.get('total_predictions', 0)}"
                    )
            else:
    ...
    def test_fallback_mechanisms(self):
        ...
        try:
            ...
            original_ml_available = (
                self.ml_engine.__class__.__module__ + ".ML_ENGINE_AVAILABLE"
            )
    ...
    def test_error_handling(self):
        ...
        try:
            ...
            result = self.ml_engine.predict(
                "INVALID", PredictionType.PRICE_DIRECTION, 5
            )
            if result is None:
            ...
            result = self.ml_engine.predict(
                "AAPL", PredictionType.PRICE_DIRECTION, 1000
            )
    ...
    def generate_final_report(self, total_tests, passed_tests):
        ...
        logger.info(
            f"   - Prediction Cache: {len(self.ml_engine.prediction_cache)} entries"
        )
        ...
        if not self.ml_engine.models:
            logger.info(
                "   💡 Consider installing ML dependencies for full functionality"
            )
