...
def test_realtime_learning_engine():
    ...
    try:
        ...
        print(f"✅ System status retrieved: {status['total_samples_processed']} samples processed")
...
def test_ml_diagnostics_system():
    ...
    try:
        ...
        print(f"✅ Drift detection completed: drift score = {drift_metrics.drift_score:.3f}")
        
        # Test system health monitoring
        models = {"test_model": model}
        health = diagnostics.monitor_system_health(models)
        print(f"✅ System health monitored: {health.trained_models}/{health.total_models} models trained")
...
def test_integration_with_ensemble():
    ...
    try:
        ...
        print(f"✅ Phase 2 fully operational: {'✅' if phase2_status['phase2_fully_operational'] else '❌'}")
