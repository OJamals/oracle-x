    ...
    try:
        ...
        if result:
            ...
            details = {
                ...
                "sample_sentiment": (
                    sample_data.get("sentiment_score", 0) if sample_data else 0
                ),
                "has_sample_texts": (
                    bool(sample_data.get("sample_texts", [])) if sample_data else False
                ),
            }
...
def test_finviz_adapter():
    ...
    try:
        ...
        if breadth:
            ...
            log_test(
                "finviz_market_breadth", "PASS", json.dumps(details), breadth_timing
            )
        else:
        ...
        if sectors and len(sectors) > 0:
            details = {
                ...
                "sample_1d_perf": (
                    float(sectors[0].perf_1d) if sectors[0].perf_1d else None
                ),
            }
            log_test(
                "finviz_sector_performance", "PASS", json.dumps(details), sector_timing
            )
        else:
            log_test(
                "finviz_sector_performance", "FAIL", "No sector data", sector_timing
            )
    ...
    try:
        ...
        if investing_id:
            log_test(
                "investiny_search",
                "PASS",
                json.dumps({"investing_id": investing_id}),
                timing,
            )
            ...
            log_test(
                "investiny_search",
                "FAIL",
                "Could not find investing_id for AAPL",
                timing,
            )

    except ImportError:
        log_test(
            "investiny_adapter", "SKIP", "Investiny adapter dependencies not available"
        )
    except Exception as e:
...
def test_adapter_wrappers():
    ...
    try:
        ...
        try:
            ...
            yf_adapter = YFinanceAdapterWrapper(
                cache_adapter, rate_limiter, performance_tracker
            )
            ...
            log_test(
                "yfinance_wrapper_capabilities",
                "PASS",
                json.dumps({"capabilities": list(capabilities)}),
                timing,
            )
        ...
        try:
            if os.getenv("FINANCIALMODELINGPREP_API_KEY"):
                fmp_adapter = FMPAdapterWrapper(
                    cache, rate_limiter, performance_tracker
                )
                ...
                log_test(
                    "fmp_wrapper",
                    "SKIP",
                    "FINANCIALMODELINGPREP_API_KEY not configured",
                )
        except Exception as e:
        ...
        try:
            if os.getenv("FINNHUB_API_KEY"):
                finnhub_adapter = FinnhubAdapterWrapper(
                    cache, rate_limiter, performance_tracker
                )
        ...
        try:
            findb_adapter = FinanceDatabaseAdapterWrapper(
                cache, rate_limiter, performance_tracker
            )
            ...
            log_test(
                "financedb_wrapper_capabilities",
                "PASS",
                json.dumps({"capabilities": list(capabilities)}),
                timing,
            )
...
def test_orchestrator_basic():
    ...
    try:
        ...
        log_test(
            "orchestrator_init",
            "PASS",
            f"Initialized in {init_timing:.2f}s",
            init_timing,
        )
    ...
    try:
        ...
        if sentiment_data:
            ...
            details = {
                ...
                "sample_score": (
                    getattr(sample_data, "sentiment_score", None)
                    if sample_data
                    else None
                ),
                "sample_confidence": (
                    getattr(sample_data, "confidence", None) if sample_data else None
                ),
            }
    ...
    try:
        ...
        if market_data and market_data.data is not None:
            details = {
                ...
                "columns": (
                    list(market_data.data.columns)
                    if hasattr(market_data.data, "columns")
                    else []
                ),
            ...
            if result.returncode == 0:
                try:
                    ...
                    if data.get("ok"):
                        ...
                        log_test(
                            cmd_name, "FAIL", data.get("error", "Unknown error"), timing
                        )
