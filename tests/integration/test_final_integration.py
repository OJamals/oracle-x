...
def test_main_pipeline_integration():
    ...
    try:
        ...
        from data_feeds.data_feed_orchestrator import get_enhanced_sentiment_analysis

        start_time = time.time()
        enhanced_result = get_enhanced_sentiment_analysis(
            test_symbol, include_reddit=False
        )
        enhanced_time = time.time() - start_time

        print(
            f"   ✅ Enhanced Sentiment: {enhanced_result.get('overall_sentiment', 0):.3f}"
        )
        ...
        for source, data in original_result.items():
            print(
                f"     • {source}: {data.sentiment_score:.3f} (confidence: {data.confidence:.3f})"
            )
        ...
        improvement = (
            ((original_time - enhanced_time) / original_time) * 100
            if original_time > 0
            else 0
        )
        if improvement > 0:
...
def test_sentiment_data_quality():
    ...
    try:
        ...
        # Calculate averages
        avg_quality = sum(m["quality_score"] for m in quality_metrics) / len(
            quality_metrics
        )
        avg_confidence = sum(m["confidence"] for m in quality_metrics) / len(
            quality_metrics
        )
        avg_sources = sum(m["sources_count"] for m in quality_metrics) / len(
            quality_metrics
        )
        avg_time = sum(m["processing_time"] for m in quality_metrics) / len(
            quality_metrics
        )
...
def test_error_handling():
    ...
    try:
        ...
        print(
            f"   Fallback Status: {'✅ Working' if result.get('sources_count', 0) > 0 else '⚠️ Limited'}"
        )
