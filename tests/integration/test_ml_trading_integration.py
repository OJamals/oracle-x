...
def test_ml_trading_integration():
    ...
    try:
        ...
        from oracle_engine.ml_trading_integration import (
            ...
            MLTradingConfig,
            create_ml_trading_orchestrator,
        )
        ...
        try:
            training_result = orchestrator.train_models(
                ["AAPL", "GOOGL"], lookback_days=100
            )
...
def test_signal_generation_edge_cases():
    ...
    try:
        ...
        print(
            f"✓ Handled minimal data case: {signal.signal_type if signal else 'None'}"
        )
        ...
        print(
            f"✓ Handled missing sentiment case: {signal.signal_type if signal else 'None'}"
        )
...
def test_ml_config_validation():
    ...
    try:
        ...
        print(
            f"✓ Default config created: min_confidence={default_config.min_confidence_threshold}"
        )
        ...
        total_weight = (
            custom_config.technical_weight
...
def test_ml_integration_without_ml():
    ...
    try:
        ...
        # Create test data
        test_data = pd.DataFrame(
            {"Close": [100, 102, 98, 105, 103], "Volume": [1000, 1100, 900, 1200, 1150]}
        )

        mock_data_orchestrator.get_historical_data = Mock(return_value=test_data)
        mock_sentiment_engine.get_sentiment_analysis = Mock(
            return_value={"overall_sentiment": 0.2, "confidence": 0.7}
        )
        ...
        if signal:
            print(
                f"✓ Fallback signal generated: {signal.signal_type} "
                f"(confidence: {signal.confidence:.3f})"
            )
        else:
