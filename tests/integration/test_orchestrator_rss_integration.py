...
try:
    ...
    def test_orchestrator_rss_integration():
            ...
            try:
                ...
                if sentiment_data_dict:
                    ...
                    for source_name, sentiment_data in sentiment_data_dict.items():
                        if 'rss' in source_name.lower() or 'news' in source_name.lower():
                            rss_sentiment = sentiment_data
                            break
                    
                    # Also check for any sentiment data as fallback
                    if not rss_sentiment and sentiment_data_dict:
                        source_name, rss_sentiment = next(iter(sentiment_data_dict.items()))
                    ...
                    if rss_sentiment:
                        ...
                        print(f"  ✅ Sentiment Score: {rss_sentiment.sentiment_score:.3f}")
                        ...
                        if hasattr(rss_sentiment, 'raw_data') and rss_sentiment.raw_data:
                            sample_texts = rss_sentiment.raw_data.get('sample_texts', [])
                            if sample_texts:
                                print(f"  📝 Sample Headlines:")
                                for i, text in enumerate(sample_texts[:3], 1):
                                    print(f"    {i}. {text[:100]}...")
                    else:
                        results[symbol] = {'success': False, 'error': 'No RSS sentiment data found'}
                        print(f"  ❌ No RSS sentiment data found for {symbol}")
                        print(f"  🔍 Available sources: {list(sentiment_data_dict.keys())}")
                else:
        ...
        if successful >= total * 0.75:  # 75% success rate threshold
            ...
            print(f"   📰 News adapters successfully integrated into DataFeedOrchestrator")
