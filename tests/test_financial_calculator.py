...
def test_financial_integration():
    ...
    if market_data is not None and not market_data.data.empty:
        ...
        print(
            f"   Rows: {len(market_data.data)}, Columns: {list(market_data.data.columns)}"
        )
    else:
    ...
    if metrics.price_change is not None:
        print(
            f"   Price Change: ${metrics.price_change} ({metrics.price_change_percent}%)"
        )
    ...
    if quotes:
        ...
        print(
            f"   Combined Market Cap: ${portfolio_metrics.get('total_market_cap', 0):,}"
        )
        ...
        if portfolio_metrics.get("avg_price_change"):
            print(
                f"   Average Price Change: {portfolio_metrics['avg_price_change']:.2f}%"
            )
