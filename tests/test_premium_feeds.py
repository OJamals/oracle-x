...
def test_premium_feeds():
    ...
    print("\n📋 API Availability:")
    print(f"   Finnhub: {'✅ Available' if premium.finnhub_available else '❌ No API key'}")
        ...
        if finnhub_data.get("available"):
            ...
            if finnhub_data.get("insider_sentiment"):
                ...
                for symbol, data in finnhub_data["insider_sentiment"].items():
                    print(f"      {symbol}: {data['sentiment']} (net: {data['net_buying']:,} shares, {data['transactions']} transactions)")
            
            # Show recommendations
            if finnhub_data.get("recommendation_trends"):
                print(f"\n   🎯 Analyst Recommendations:")
                for symbol, data in finnhub_data["recommendation_trends"].items():
                    print(f"      {symbol}: {data['consensus']} (buy: {data['buy']}, hold: {data['hold']}, sell: {data['sell']})")
            ...
            if finnhub_data.get("price_targets"):
                ...
                for symbol, data in finnhub_data["price_targets"].items():
                    ...
                    if mean:
                        print(f"      {symbol}: ${mean:.2f} avg (${data['target_low']:.2f}-${data['target_high']:.2f}, {data['num_analysts']} analysts)")
        ...
        if fmp_data.get("available"):
            ...
            if fmp_data.get("analyst_estimates"):
                ...
                for symbol, data in fmp_data["analyst_estimates"].items():
                    ...
                    if eps:
                        print(f"      {symbol}: EPS ${eps:.2f} (${data['estimated_eps_low']:.2f}-${data['estimated_eps_high']:.2f})")
                    if rev:
                        print(f"         Revenue: ${rev/1e9:.2f}B ({data['num_analysts']} analysts)")
            ...
            if fmp_data.get("institutional_ownership"):
                ...
                for symbol, data in fmp_data["institutional_ownership"].items():
                    print(f"      {symbol}: Top 5 hold {data['top_5_shares']:,} shares ({data['total_holders']} total holders)")
                    print(f"         Top holders: {', '.join(data['top_5_holders'][:3])}")
            ...
            if fmp_data.get("insider_transactions"):
                ...
                for symbol, data in fmp_data["insider_transactions"].items():
                    print(f"      {symbol}: {data['net_sentiment']} (buys: {data['recent_buys']}, sells: {data['recent_sells']})")
            ...
            if fmp_data.get("dcf_valuations"):
                ...
                for symbol, data in fmp_data["dcf_valuations"].items():
                    ...
                    if dcf and price:
                        ...
                        print(f"      {symbol}: Fair value ${dcf:.2f} vs Price ${price:.2f} ({upside:+.1f}% upside)")
    ...
    print(f"   Rate limiting: {'✅ Working' if elapsed > 1 else '⚠️  Very fast (may not be limiting)'}")
