...
def test_ml_training_with_sentiment():
    ...
    try:
        ...
        print(
            f"   📈 Training Summary: {success_count} successful, {error_count} errors"
        )

        # Test prediction with sentiment
        print("4. Testing ML prediction with sentiment...")
        prediction = ml_engine.predict(
            symbol, PredictionType.PRICE_DIRECTION, horizon_days=5
        )
        ...
        for model_key, perf in performance.items():
            print(
                f"      {model_key}: Weight={perf['weight']:.3f}, Accuracy={perf['accuracy']:.3f}"
            )
