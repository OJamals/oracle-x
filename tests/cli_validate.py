...
def _df_last_row_summary(df) -> Optional[Dict[str, Any]]:
    try:
        ...
        return {
            ...
            "Close": get("Close"),
            "AdjClose": (
                get("Adj Close") if "Adj Close" in df.columns else get("AdjClose")
            ),
            "Volume": get("Volume"),
        }
    except Exception:
    ...
    try:
        ...
        if preferred:
            ...
            quote = (
                _get_orch()
                .get_orchestrator()
                .get_quote(sym, preferred_sources=preferred)
            )
        else:
    ...
    try:
        if preferred:
            md = (
                ...
                .get_orchestrator()
                .get_market_data(
                    sym, period=period, interval=interval, preferred_sources=preferred
                )
            )
        else:
        ...
        if not md or getattr(md, "data", None) is None:
            out(
                {"ok": False, "cmd": "market_data", "symbol": sym, "error": "no data"},
                1,
            )
        ...
        payload = {
            ...
            "rows": (
                int(getattr(md.data, "shape", [0, 0])[0])
                if getattr(md, "data", None) is not None
                else 0
            ),
        }
        out(payload, 0)
...
def cmd_company_info(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if not info:
            out(
                {"ok": False, "cmd": "company_info", "symbol": sym, "error": "no data"},
                1,
            )
        payload = {
            "ok": True,
        ...
        titles = [
            {
                ...
                "published": _dt_iso(getattr(n, "published", None)),
            }
            for n in items
        ]
        payload = {
...
def cmd_advanced_sentiment(args: argparse.Namespace) -> None:
    ...
    try:
            ...
            out(
                {
                    ...
                    "symbol": sym,
                    "note": "skipped (missing key or no texts available)",
                },
                0,
            )
        payload = {
        ...
        out(
            {"ok": False, "cmd": "advanced_sentiment", "symbol": sym, "error": str(e)},
            2,
        )
            ...
            out(
                {
                    "ok": True,
                    "cmd": "market_breadth",
                    "note": "skipped (unavailable adapter or missing key)",
                },
                0,
            )
        payload = {
            ...
            "timestamp": (
                _dt_iso(getattr(breadth, "timestamp", None))
                if hasattr(breadth, "timestamp")
                else None
            ),
        }
        out(payload, 0)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--json", action="store_true", help="Output full JSON without status messages"
    )
    p.add_argument(
        "--exit-zero-even-on-error",
        action="store_true",
        help="Exit with code 0 even on error for test continuity",
    )
    ...
    sp.add_argument(
        "--preferred_sources",
        required=False,
        help="Comma-separated list, e.g., yfinance,twelve_data",
    )
    ...
    sp.add_argument(
        "--symbols", required=True, help="Comma-separated symbols, e.g., AAPL,MSFT,SPY"
    )
    sp.set_defaults(func=cmd_multiple_quotes)

    # financial_statements
    sp = sub.add_parser(
        "financial_statements", help="Get financial statements if available"
    )
    sp.add_argument("--symbol", required=True)
    ...
    # advanced_sentiment
    sp = sub.add_parser(
        "advanced_sentiment", help="Get advanced sentiment if texts available"
    )
    sp.add_argument("--symbol", required=True)
