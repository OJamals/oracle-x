...
def check_current_prices():
    ...
    for ticker in tickers:
        if ticker in current_prices:
            ...
            entry_mid = float(pipeline['entry'].split('-')[0]) + (float(pipeline['entry'].split('-')[1]) - float(pipeline['entry'].split('-')[0]))/2
            
            print(f"\n{ticker}:")
            print(f"  Current Price: ${current:.2f}")
            print(f"  Pipeline Entry: {pipeline['entry']} (mid: ${entry_mid:.2f})")
            print(f"  Difference: ${current - entry_mid:.2f} ({((current - entry_mid)/entry_mid)*100:.1f}%)")
