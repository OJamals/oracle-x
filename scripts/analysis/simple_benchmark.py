...
def benchmark_database():
    ...
    for _ in range(50):
        results = execute_query(
            db_path, "SELECT * FROM test_data WHERE symbol = ?", ("AAPL",)
        )
...
def benchmark_http():
    ...
    test_urls = ["https://httpbin.org/get", "https://httpbin.org/json"]
    ...
    return {
        "http_requests": {
            ...
            "throughput": (
                (successful_requests) / http_time if successful_requests > 0 else 0
            ),
        }
    }
...
def main():
    ...
    try:
        ...
        output = {
            ...
            "results": results,
            "summary": {
                "database_throughput": results.get("database_queries", {}).get(
                    "throughput", 0
                ),
                "http_throughput": results.get("http_requests", {}).get(
                    "throughput", 0
                ),
                "memory_efficiency": results.get("memory_processing", {}).get(
                    "processing_time", 0
                ),
            },
        }
        ...
        print("\nBenchmark Results:")
        print(
            f"Database throughput: {results['database_queries']['throughput']:.1f} queries/sec"
        )
        print(
            f"HTTP throughput: {results['http_requests']['throughput']:.1f} requests/sec"
        )
        print(
            f"Memory processing time: {results['memory_processing']['processing_time']:.3f}s"
        )
