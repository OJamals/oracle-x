...
class BenchmarkResults:
    ...
    def record_metric(self, test_name: str, metric_name: str, value: float, test_type: str = 'optimized'):
    ...
    def get_improvement(self, test_name: str, metric_name: str) -> Optional[float]:
        ...
        if (test_name in self.results and
            'baseline' in self.results[test_name] and
            'optimized' in self.results[test_name] and
            metric_name in self.results[test_name]['baseline'] and
            metric_name in self.results[test_name]['optimized']):
    ...
    def generate_summary(self) -> Dict[str, Any]:
        ...
        for test_name in self.results:
            ...
            for metric_name in ['avg_response_time', 'throughput', 'memory_usage', 'cpu_usage']:
                improvement = self.get_improvement(test_name, metric_name)
                if improvement is not None:
                    summary['test_details'][test_name][f'{metric_name}_improvement'] = improvement
...
class DatabaseBenchmark:
    ...
    def benchmark_queries(self, test_type: str = 'optimized'):
        ...
        for _ in range(ITERATIONS):
            for symbol in TEST_SYMBOLS:
                ...
                results = execute_query(self.db_path,
        ...
        self.results.record_metric('database_queries', 'total_time', total_time, test_type)
        self.results.record_metric('database_queries', 'avg_response_time', total_time / total_queries, test_type)
        self.results.record_metric('database_queries', 'throughput', total_queries / total_time, test_type)

        print(f"Database query performance: {total_time:.2f}s for {total_queries} queries")
    ...
    def benchmark_inserts(self, test_type: str = 'optimized'):
        ...
        self.results.record_metric('database_inserts', 'total_time', total_time, test_type)
        self.results.record_metric('database_inserts', 'throughput', total_inserts / total_time, test_type)

        print(f"Database insert performance: {total_time:.2f}s for {total_inserts} inserts")
    ...
    def run_baseline_database_test(self):
        ...
        self.results.record_metric('database_queries', 'total_time', total_time, 'baseline')
        self.results.record_metric('database_queries', 'avg_response_time', total_time / total_queries, 'baseline')
        self.results.record_metric('database_queries', 'throughput', total_queries / total_time, 'baseline')

        print(f"Database (baseline) query performance: {total_time:.2f}s for {total_queries} queries")
...
class HTTPBenchmark:
    ...
    def benchmark_http_requests(self, test_type: str = 'optimized'):
        ...
        self.results.record_metric('http_requests', 'avg_response_time', total_time / total_requests, test_type)
        self.results.record_metric('http_requests', 'throughput', total_requests / total_time, test_type)
        self.results.record_metric('http_requests', 'success_rate', successful_requests / total_requests * 100, test_type)

        print(f"HTTP request performance: {total_time:.2f}s for {total_requests} requests")
    ...
    async def benchmark_concurrent_http(self, test_type: str = 'optimized'):
        ...
        for symbol in TEST_SYMBOLS:
            test_requests.extend([
                {"method": "GET", "url": f"https://httpbin.org/get?symbol={symbol}&i={i}"}
        ...
        successful_requests = sum(1 for r in results if r is not None and getattr(r, 'status_code', None) == 200)

        self.results.record_metric('concurrent_http', 'total_time', total_time, test_type)
        self.results.record_metric('concurrent_http', 'throughput', len(test_requests) / total_time, test_type)
        self.results.record_metric('concurrent_http', 'success_rate', successful_requests / len(test_requests) * 100, test_type)

        print(f"Concurrent HTTP performance: {total_time:.2f}s for {len(test_requests)} requests")
    ...
    def benchmark_dataframe_processing(self, test_type: str = 'optimized'):
        ...
        if test_type == 'optimized':
            ...
            result = self.memory_processor.process_large_dataframe(
                optimized_df,
                lambda chunk: chunk.groupby('symbol')['price'].mean()
            )
        else:
        ...
        self.results.record_metric('dataframe_processing', 'processing_time', processing_time, test_type)
        self.results.record_metric('dataframe_processing', 'memory_usage', memory_used, test_type)
        self.results.record_metric('dataframe_processing', 'peak_memory_mb', memory_after, test_type)

        print(f"DataFrame processing: Took {processing_time:.2f}s with {memory_used:.2f}MB memory usage")
    ...
    def benchmark_end_to_end_pipeline(self, test_type: str = 'optimized'):
        ...
        try:
            ...
            for symbol in TEST_SYMBOLS[:3]:  # Test with fewer symbols for end-to-end
                try:
                    # Get market data
                    market_data = orchestrator.get_market_data(symbol, period="1d", interval="1h")
        ...
        self.results.record_metric('end_to_end_pipeline', 'total_time', total_time, test_type)
        self.results.record_metric('end_to_end_pipeline', 'avg_response_time', total_time / len(TEST_SYMBOLS[:3]), test_type)
        self.results.record_metric('end_to_end_pipeline', 'success_rate', successful_requests / total_requests * 100, test_type)

        print(f"End-to-end pipeline: {total_time:.2f}s with {successful_requests}/{total_requests} successful requests")
...
def main():
    parser = argparse.ArgumentParser(description="Run comprehensive performance benchmarks for Oracle-X.")
    parser.add_argument("--baseline", action="store_true", help="Run baseline benchmarks only")
    parser.add_argument("--optimized", action="store_true", help="Run optimized benchmarks only")
    parser.add_argument("--all", action="store_true", help="Run both baseline and optimized benchmarks")
