...
def test_fixed_training():
    ...
    try:
        ...
        for training_type, results in training_results.items():
            ...
            if isinstance(results, dict):
                ...
                if model_count > 0:
                    ...
                    print(
                        f"    📈 Successfully trained {model_count} models for {training_type}"
                    )
            else:
        ...
        ):  # At least 4 models (2 types × 2 algorithms minimum)
            print("\n🎉 PHASE 1 INTEGRATION: SUCCESS!")
            print(
                "   The fixed training system is working in the main oracle-x system!"
            )
            return True
        else:
...
def test_ensemble_functionality():
    ...
    try:
        ...
        try:
            direction_pred = engine.predict(
                "AAPL", PredictionType.PRICE_DIRECTION, horizon_days=1
            )
            if direction_pred:
        ...
        try:
            target_pred = engine.predict(
                "AAPL", PredictionType.PRICE_TARGET, horizon_days=1
            )
            if target_pred:
