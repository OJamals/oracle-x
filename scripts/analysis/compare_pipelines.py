...
def run_enhanced_pipeline_test(symbols):
    ...
    try:
        ...
        test_features = pipeline.feature_engine.extract_features(
            "AAPL", test_market_data
        )
        features_count = len(
            [
                attr
                for attr in dir(test_features)
                if not attr.startswith("_") and getattr(test_features, attr) is not None
            ]
        )
...
def compare_accuracy_features():
    ...
    original_features = {
        "Technical Indicators": ["Basic RSI", "Simple Moving Average", "Price Change"],
        "Volatility Analysis": ["Basic Historical Volatility"],
        "Volume Analysis": ["Simple Volume"],
        "Options Analytics": ["Basic Option Pricing"],
        "Risk Metrics": ["Basic Position Sizing"],
        "Machine Learning": ["Simple Ensemble", "Basic Features"],
    }
    ...
    for category in original_features:
        print(f"\n{category}:")
        print(
            f"  Original ({len(original_features[category])}): {', '.join(original_features[category][:3])}{'...' if len(original_features[category]) > 3 else ''}"
        )
        print(
            f"  Enhanced ({len(enhanced_features[category])}): {', '.join(enhanced_features[category][:3])}{'...' if len(enhanced_features[category]) > 3 else ''}"
        )
    ...
    for category in original_features:
        ...
        improvement_ratio = (
            enhanced_count / original_count if original_count > 0 else float("inf")
        )
        print(
            f"  {category}: {improvement_ratio:.1f}x improvement ({original_count} → {enhanced_count})"
        )
...
def run_comprehensive_comparison():
    ...
    for metric_name, metric_key, metric_type in comparison_metrics:
        ...
        if metric_type == "boolean":
            ...
            if original_val > 0:
                improvement = (
                    f"{(original_val/enhanced_val):.1f}x faster"
                    if enhanced_val < original_val
                    else f"{(enhanced_val/original_val):.1f}x slower"
                )
            else:
                improvement = "N/A"
        else:  # count
        ...
        print(
            f"{metric_name:<25} {original_str:<15} {enhanced_str:<15} {improvement:<15}"
        )
    ...
    if original_results.get("accuracy_features"):
        ...
        if len(original_results["accuracy_features"]) > 10:
            print(
                f"      ... and {len(original_results['accuracy_features']) - 10} more"
            )
    else:
    ...
    if enhanced_results.get("accuracy_features"):
        ...
        if len(enhanced_results["accuracy_features"]) > 10:
            print(
                f"      ... and {len(enhanced_results['accuracy_features']) - 10} more"
            )
    ...
    comparison_data = {
        ...
        "summary": {
            "feature_improvement": (
                f"{enhanced_feature_count/original_feature_count:.1f}x"
                if original_feature_count > 0
                else "∞x"
            ),
            "execution_improvement": (
                f"{original_results['execution_time']/enhanced_results['execution_time']:.1f}x faster"
                if enhanced_results["execution_time"] > 0
                and original_results["execution_time"] > 0
                else "N/A"
            ),
            "reliability_improvement": (
                "Initialization issues fixed"
                if enhanced_results["initialization_success"]
                and not original_results["initialization_success"]
                else "Same"
            ),
        },
    }
    ...
    output_file = (
        f"pipeline_comparison_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
