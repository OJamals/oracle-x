    ...
    mock_data = [
        {
            ...
            "scores": {"opportunity": 75.2, "ml_confidence": 0.72, "valuation": 0.15},
            "trade": {
                "entry_price": 2.90,
                "target_price": 4.20,
                "stop_loss": 2.03,
        ...
        },
        {
            ...
            "scores": {"opportunity": 82.5, "ml_confidence": 0.78, "valuation": 0.23},
            "trade": {
                "entry_price": 8.80,
                "target_price": 13.50,
                "stop_loss": 6.16,
        ...
        },
        {
            ...
            "scores": {"opportunity": 68.3, "ml_confidence": 0.65, "valuation": 0.12},
            "trade": {
                "entry_price": 3.28,
                "target_price": 5.10,
                "stop_loss": 2.30,
...
def main():
    ...
    try:
        ...
        pipeline, init_error = safe_execute_with_timeout(
            init_pipeline, timeout_seconds=15
        )
        ...
        if init_error:
            ...
            display_recommendations(
                mock_recommendations, "DEMONSTRATION: TOP OPTIONS FOR AUGUST 20, 2025"
            )
            ...
            for i, rec in enumerate(mock_recommendations, 1):
                ...
                print(
                    f'Contract: {rec["contract"]["type"].upper()} ${rec["contract"]["strike"]} {rec["contract"]["expiry"]}'
                )
        ...
        if scan_error or result is None:
            ...
            print(
                "📊 Live data feeds may be unavailable. Showing demonstration mode..."
            )

            # Show mock results
            mock_recommendations = create_mock_recommendations()
            display_recommendations(
                mock_recommendations[:2], "DEMONSTRATION: OPTIONS ANALYSIS RESULTS"
            )
            ...
            if result.recommendations:
                ...
                for i, rec in enumerate(result.recommendations[:3], 1):
                    print(
                        f"#{i} - {rec.symbol} {rec.contract.option_type.value.upper()}"
                    )
                    print(f"   Strike: ${rec.contract.strike:.2f}")
                ...
                mock_recommendations = create_mock_recommendations()
                display_recommendations(
                    mock_recommendations[:2], "FALLBACK: DEMONSTRATION OPTIONS"
                )
        ...
        mock_recommendations = create_mock_recommendations()
        display_recommendations(
            mock_recommendations[:2], "DEMONSTRATION: PIPELINE CAPABILITIES"
        )
