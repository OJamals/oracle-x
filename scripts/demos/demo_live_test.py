...
def demo_valuation_engine():
    ...
    fair_value, model_prices = engine.calculate_fair_value(
        aapl_call, underlying_price=228.0, volatility=0.28, dividend_yield=0.005
    )
    ...
    result = engine.detect_mispricing(
        aapl_call, underlying_price=228.0, market_data=market_data
    )
    ...
    expected_return, prob_profit = engine.calculate_expected_returns(
        result, target_price=235.0, probability_model="normal"  # Target stock price
    )
...
def demo_data_feeds():
    ...
    try:
        analytics = orchestrator.get_options_analytics(
            "AAPL", include=["chain", "gex", "max_pain"]
        )
        if analytics:
...
def demo_opportunity_scoring():
    ...
    opportunities = engine.scan_opportunities(
        options_chain, underlying_price=228.0, min_opportunity_score=0
    )
