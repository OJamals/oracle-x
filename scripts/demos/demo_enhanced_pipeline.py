...
def demonstrate_enhanced_features():
    ...
    for symbol in test_symbols:
        ...
        print(
            f"  Sharpe Ratio: {features.sharpe_ratio:.2f}"
            if features.sharpe_ratio
            else "  Sharpe Ratio: N/A"
        )
        print(
            f"  VaR (1d): {features.var_1d:.3f}"
            if features.var_1d
            else "  VaR (1d): N/A"
        )
    ...
    for symbol in test_symbols:
        ...
        if recommendations:
            ...
            print(
                f"  Top opportunity: {top_rec['strategy']} {top_rec['contract']['type']}"
            )
    ...
    start_time = time.time()
    scan_results = pipeline.generate_market_scan(
        scan_symbols, max_symbols=len(scan_symbols)
    )
    ...
    if scan_results["top_opportunities"]:
        ...
        for i, opp in enumerate(scan_results["top_opportunities"][:3], 1):
            print(
                f"  {i}. {opp['symbol']} {opp['strategy']} - Score: {opp['scores']['opportunity']:.1f}"
            )
            print(
                f"     Strike: ${opp['contract']['strike']:.2f}, Entry: ${opp['trade']['entry_price']:.2f}"
            )
    ...
    if "market_insights" in scan_results:
        ...
        print(
            f"  Average ML Confidence: {insights.get('average_ml_confidence', 'N/A')}"
        )
    ...
    print(
        f"  ML models available: {performance['ml_engine_stats']['models_available']}"
    )
...
def run_performance_benchmark():
    ...
    simple_config = EnhancedPipelineConfig(
        model_complexity=ModelComplexity.SIMPLE, max_workers=2
    )
    ...
    for config_name, config in [
        ...
        print(
            f"  Opportunities found: {results['scan_results']['opportunities_found']}"
        )
