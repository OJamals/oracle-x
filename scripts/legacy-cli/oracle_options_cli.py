...
def print_recommendation(rec: Dict[str, Any], verbose: bool = False):
    ...
    print(
        f"   Strike: ${rec['contract']['strike']:.2f} | Expiry: {rec['contract']['expiry'][:10]}"
    )
    print("-" * 60)
    ...
    print(
        f"   Target Price: ${trade['target_price']:.2f} ({(trade['target_price']/trade['entry_price']-1)*100:.1f}%)"
    )
    print(
        f"   Stop Loss: ${trade['stop_loss']:.2f} ({(trade['stop_loss']/trade['entry_price']-1)*100:.1f}%)"
    )
...
def cmd_scan(args):
    ...
    if args.output:
        output_data = {
            ...
            "recommendations": [
                r.to_dict() for r in result.recommendations[: args.top]
            ],
        }
        with open(args.output, "w") as f:
            json.dump(output_data, f, indent=2)
...
Examples:
  ...
  %(prog)s monitor positions.json --output updates.json
    ...
    analyze_parser.add_argument(
        "--expiry", type=str, help="Specific expiry date (YYYY-MM-DD)"
    )
    analyze_parser.add_argument(
        "--limit", type=int, default=5, help="Number of recommendations to show"
    )
    analyze_parser.add_argument(
        "--min-score", type=float, default=70, help="Minimum opportunity score"
    )
    analyze_parser.add_argument(
        "--risk",
        type=str,
    ...
    analyze_parser.add_argument(
        "--min-days", type=int, default=7, help="Minimum days to expiry"
    )
    analyze_parser.add_argument(
        "--max-days", type=int, default=90, help="Maximum days to expiry"
    )
    analyze_parser.add_argument(
    ...
    scan_parser.add_argument(
        "--symbols", type=str, help="Comma-separated list of symbols"
    )
    scan_parser.add_argument(
        "--max-symbols", type=int, default=20, help="Maximum symbols to scan"
    )
    scan_parser.add_argument(
        "--top", type=int, default=10, help="Number of top opportunities to show"
    )
    scan_parser.add_argument(
        "--min-score", type=float, default=70, help="Minimum opportunity score"
    )
    scan_parser.add_argument(
        "--min-days", type=int, default=3, help="Minimum days to expiry"
    )
    scan_parser.add_argument(
        "--max-days", type=int, default=120, help="Maximum days to expiry"
    )
    scan_parser.add_argument(
        "--risk",
        type=str,
    ...
    monitor_parser.add_argument(
        "positions_file", type=str, help="JSON file with positions"
    )
