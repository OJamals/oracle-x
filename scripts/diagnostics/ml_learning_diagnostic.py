...
def test_ml_engine_initialization():
    ...
    try:
        ...
        engine = EnsemblePredictionEngine(
            data_orchestrator=orchestrator, sentiment_engine=sentiment_engine
        )
    ...
    try:
        ...
        for symbol in symbols:
            ...
            if (
                ...
                logger.info(
                    f"Market data collection successful: {data.shape} rows/columns"
                )
                ...
                logger.info(
                    f"Missing data per column: {missing_data[missing_data > 0].to_dict()}"
                )
...
def test_prediction_pipeline():
    ...
    try:
        ...
        prediction = engine.predict(
            "AAPL", PredictionType.PRICE_DIRECTION, horizon_days=5
        )
...
def analyze_learning_capabilities():
    ...
    try:
        ...
        try:
            ...
            capabilities["online_learning"] = hasattr(rf_model, "update") and callable(
                rf_model.update
            )
        except:
        ...
        try:
            ...
            capabilities["performance_monitoring"] = hasattr(
        ...
        try:
            perf = ModelPerformance(
                model_name="test", prediction_type=PredictionType.PRICE_DIRECTION
            )
...
def run_comprehensive_diagnostic():
    ...
    if init_results.get("initialization_success"):
        ...
        print(
            f"❌ Engine initialization failed: {init_results.get('error', 'Unknown error')}"
        )
    ...
    if data_results and data_results.get("data_collection_success"):
        ...
        error_msg = (
            data_results.get("error", "Unknown error")
            if data_results
            else "No data results returned"
        )
    ...
    for symbol, result in sentiment_results.items():
        if result.get("success"):
            print(
                f"✅ {symbol}: sentiment={result.get('sentiment', 'N/A'):.3f}, confidence={result.get('confidence', 'N/A'):.3f}"
            )
        else:
    ...
    overall_score = (
        (health_score / total_checks + capability_score / total_capabilities) / 2 * 100
    )
