    ...
    try:
        ...
        if engine.feature_engineer:
            ...
            try:
                ...
                target_cols = [
                    col for col in features_df.columns if col.startswith("target_")
                ]
                ...
                for target_col in target_cols:
                    ...
                    if len(valid_targets) > 0:
                        print(
                            f"    Range: {valid_targets.min():.4f} to {valid_targets.max():.4f}"
                        )
...
def diagnose_model_training(features_df):
    ...
    try:
        ...
        for prediction_type in [
            ...
            relevant_models = {
                k: v for k, v in engine.models.items() if prediction_type.value in k
            }
            ...
            for model_key, model in relevant_models.items():
                ...
                try:
                    ...
                    print(
                        f"  Pre-training state: trained={getattr(model, 'is_trained', 'unknown')}"
                    )
                    ...
                    print(
                        f"  Post-training state: trained={getattr(model, 'is_trained', 'unknown')}"
                    )
                    ...
                    if getattr(model, "is_trained", False):
                        try:
                            ...
                            print(
                                f"  ✅ Prediction test successful: shape={np.array(pred).shape}"
                            )
...
def diagnose_ml_model_internals():
    ...
    try:
        ...
        try:
            ...
            print(
                f"🔧 Testing model creation: {model_type.value}_{prediction_type.value}"
            )
