...
def example_market_scan():
    ...
    if result.recommendations:
        ...
        for i, rec in enumerate(result.recommendations[:5], 1):
            ...
            print(
                f"   Strike: ${rec.contract.strike:.2f} | Expiry: {rec.contract.expiry.strftime('%m/%d')}"
            )
            print(
                f"   Score: {rec.opportunity_score:.1f} | Entry: ${rec.entry_price:.2f}"
            )
            print(f"   Expected Return: {rec.expected_return:.1%}")
...
def example_generate_recommendations():
    ...
    if recommendations:
        ...
        print(
            f"Contract: {rec['contract']['type'].upper()} ${rec['contract']['strike']:.2f}"
        )
...
def example_advanced_filtering():
    ...
    for symbol in symbols:
        ...
        filtered = [
            r for r in recs if r.ml_confidence > 0.75 and r.probability_of_profit > 0.6
        ]
