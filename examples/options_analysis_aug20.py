...
def display_detailed_recommendation(rec, show_full_analysis=True):
    ...
    print(
        f"      {contract['type']} ${contract['strike']:.2f} exp {contract['expiry']}"
    )
    ...
    if show_full_analysis:
        ...
        print(
            f"      Support: {', '.join([f'${x:.2f}' for x in tech['support_levels']])}"
        )
        print(
            f"      Resistance: {', '.join([f'${x:.2f}' for x in tech['resistance_levels']])}"
        )
...
def display_portfolio_allocation(recommendations):
    ...
    for rec in recommendations:
        allocation = float(
            rec["trade_setup"]["position_size"].replace("% of portfolio", "")
        )
...
def main():
    ...
    for rec in recommendations:
        ...
        print(
            f"   Score: {rec['analysis']['opportunity_score']:.1f} | "
