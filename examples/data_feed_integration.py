...
from scripts.consolidated_data_feed import (
...
def demonstrate_basic_usage():
    ...
    if quote:
        print(
            f"AAPL: ${quote.price:.2f} ({quote.change:+.2f}, {quote.change_percent:+.2f}%)"
        )
...
def demonstrate_portfolio_analysis():
    ...
    print(
        f"{'Symbol':<6} {'Shares':<8} {'Price':<10} {'Value':<12} {'Change':<10} {'Source':<15}"
    )
    print("-" * 75)
    ...
    for symbol, shares in portfolio.items():
        if symbol in quotes:
            ...
            print(
                f"{symbol:<6} {shares:<8} ${quote.price:<9.2f} ${value:<11.2f} "
    ...
    print(
        f"Total Change: ${total_change:+.2f} ({(total_change/total_value)*100:+.2f}%)"
    )
    print()
