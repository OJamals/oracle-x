...
def test_pipeline_integration():
    ...
    try:
        ...
        print(
            f"✅ Optimization engine initialized with {len(engine.prompt_templates)} templates"
        )

        # Test agent initialization
        agent = get_optimized_agent()
        print(
            f"✅ Optimized agent initialized (optimization enabled: {agent.optimization_enabled})"
        )

        # Test analytics
        analytics = get_optimization_analytics()
        print(
            f"✅ Analytics system functional (total templates: {analytics.get('total_templates', 0)})"
        )
...
def generate_integration_report():
    ...
    try:
        ...
        report["system_status"]["integration_test"] = (
    ...
    for file in created_files:
        if os.path.exists(file):
            report["files_created"].append(
                {"filename": file, "size_bytes": os.path.getsize(file), "created": True}
            )

    # Configuration summary
    report["configuration"] = {
        "optimization_enabled": os.getenv("ORACLE_OPTIMIZATION_ENABLED", "false")
        == "true",
        "experiments_enabled": os.getenv(
            "ORACLE_OPTIMIZATION_EXPERIMENTS_ENABLED", "false"
        )
        == "true",
        "learning_enabled": os.getenv("ORACLE_OPTIMIZATION_LEARNING_ENABLED", "false")
        == "true",
...
def main():
    ...
    try:
        ...
        if args.report or args.all:
            ...
            print("\n📋 Integration Summary:")
            print(
                f"   Status: {'✅ Ready' if report['system_status'].get('optimization_ready') else '❌ Not Ready'}"
            )
            print(f"   Files Created: {len(report['files_created'])}")
            print(
                f"   Configuration: {'✅ Enabled' if report['configuration']['optimization_enabled'] else '❌ Disabled'}"
            )
