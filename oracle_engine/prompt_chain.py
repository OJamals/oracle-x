...
def get_signals_from_scrapers(prompt_text: str, chart_image_b64: str, enable_premium: bool = True, optimize: bool = False) -> Dict[str, Any]:
    """
...
def adjust_scenario_tree(signals: Dict[str, Any], similar_scenarios: str, model_name: str = MODEL_NAME, optimize: bool = False, template_id: Optional[str] = None) -> str:
    """
    Calls the LLM to produce an adjusted scenario tree with improved probabilities.
    Returns the LLM's response as a string.
    """
    if optimize:
        content, _ = _adjust_scenario_tree_optimized(signals, similar_scenarios, model_name, template_id)
        return content
...
def generate_final_playbook(signals, scenario_tree, model_name=MODEL_NAME, optimize: bool = False, template_id: Optional[str] = None):
    """
    Final LLM step: generates tomorrow's best trades + 'Tomorrow's Tape'.
    """
    if optimize:
        content, _ = _generate_final_playbook_optimized(signals, scenario_tree, model_name, template_id)
        return content
