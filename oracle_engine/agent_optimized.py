...
class OracleAgentOptimized:
        ...
        try:
            ...
            if self.optimization_enabled:
                signals = get_signals_from_scrapers(
                    prompt_text, chart_image_b64 or "", optimize=True
                )
            else:
            ...
            logger.info(
                f"✅ Collected {len(signals)} signals in {metadata['stages']['signal_collection']['duration']:.2f}s"
            )
            ...
            metadata["stages"]["scenario_retrieval"] = {
                "duration": time.time() - stage_start,
                "scenarios_found": (
                    len(similar_scenarios.split("\n")) if similar_scenarios else 0
                ),
            }

            logger.info(
                f"✅ Retrieved scenarios in {metadata['stages']['scenario_retrieval']['duration']:.2f}s"
            )
            ...
            logger.info(
                f"✅ Generated scenario tree in {metadata['stages']['scenario_tree']['duration']:.2f}s"
            )
            ...
            logger.info(
                f"✅ Generated playbook in {metadata['stages']['playbook_generation']['duration']:.2f}s"
            )
            ...
            if enable_experiments and self.optimization_enabled:
                ...
                logger.info(
                    f"✅ Completed experiments in {metadata['stages']['experiments']['duration']:.2f}s"
                )
    ...
    def batch_pipeline_optimized(
        ...
        logger.info(
            f"🚀 Starting optimized batch pipeline for {len(prompt_texts)} items"
        )

        for i, (prompt_text, chart_image) in enumerate(
            zip(prompt_texts, chart_image_b64s)
        ):
        ...
        logger.info(
            f"✅ Batch processing completed. Template performance: {template_performance}"
        )
        return results

    def _select_template_for_batch(
        self, template_performance: Dict, batch_index: int
    ) -> Optional[str]:
    ...
    def _record_pipeline_performance(self, metadata: Dict[str, Any], output: str):
        ...
        performance_record = {
            ...
            "market_condition": metadata.get("stages", {})
            .get("signal_collection", {})
            .get("market_condition"),
            "template_used": metadata.get("stages", {})
            .get("playbook_generation", {})
            .get("optimization_metadata", {})
            .get("template_used"),
        }
    ...
    def get_performance_summary(self, days: int = 7) -> Dict[str, Any]:
        ...
        if recent_history:
            success_rate = sum(1 for r in recent_history if r["success"]) / len(
                recent_history
            )
            avg_duration = sum(r["total_duration"] for r in recent_history) / len(
                recent_history
            )
    ...
    def run_learning_cycle(self, performance_threshold: float = 0.7) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"✅ Learning cycle completed. Evolved {len(evolved_templates)} templates"
            )
            return result
...
def oracle_agent_pipeline_optimized(
    prompt_text: str, chart_image_b64: Optional[str]
) -> str:
    ...
    playbook, metadata = agent.oracle_agent_pipeline_optimized(
        prompt_text, chart_image_b64
    )
    return playbook


def oracle_agent_batch_pipeline_optimized(
    prompt_texts: List[str], chart_image_b64s: List[str]
) -> List[str]:
