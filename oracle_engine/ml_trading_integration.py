...
class MLTradingOrchestrator:
    ...
    def __init__(
        ...
        self.ml_engine = create_prediction_engine(data_orchestrator, sentiment_engine)
    ...
    def generate_trading_signal(self, symbol: str) -> Optional[MLTradingSignal]:
        ...
        try:
            ...
            target_pred = self.ml_engine.predict(
                symbol, PredictionType.PRICE_TARGET, self.config.target_horizon_days
            )

            volatility_pred = self.ml_engine.predict(
                symbol, PredictionType.VOLATILITY, self.config.volatility_horizon_days
            )
            ...
            signal = MLTradingSignal(
                ...
                market_regime=(
                    direction_pred.market_regime if direction_pred else "normal"
                ),
            ...
            logger.info(
                f"Generated signal for {symbol}: {signal.signal_type} "
    ...
    def _calculate_technical_signal(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        ) * 100  # Percentage above/below SMA
    ...
    def _calculate_ml_signal(
        self,
        direction_pred: Optional[PredictionResult],
        target_pred: Optional[PredictionResult],
    ) -> Dict[str, Any]:
    ...
    def _combine_signals(
        self, technical: Dict, sentiment: Dict, ml: Dict
    ) -> Dict[str, Any]:
        ...
        technical_score = (
            signal_to_score(technical["type"])
            * technical["strength"]
            * self.config.technical_weight
        )
        sentiment_score = (
            signal_to_score(sentiment["type"])
            * sentiment["strength"]
    ...
    def _calculate_risk_score(
        ...
        data: pd.DataFrame,
        direction_pred: Optional[PredictionResult],
        volatility_pred: Optional[PredictionResult],
    ) -> float:
