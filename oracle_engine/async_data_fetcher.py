...
async def fetch_earnings_calendar_async(tickers: List[str]) -> List[Dict[str, Any]]:
    ...
    try:
        logger.debug(
            f"[ASYNC] Fetching earnings calendar for {len(tickers)} tickers..."
        )
        start_time = asyncio.get_event_loop().time()
...
async def get_signals_from_scrapers_async(
    prompt_text: str, chart_image_b64: Optional[str] = None, enable_premium: bool = True
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if dark_pools_enhanced:
            ...
            logger.info(
                f"✅ Enhanced dark pool detection: {len(dark_pools_enhanced)} signals"
            )
    except Exception as e:
...
if __name__ == "__main__":
    ...
    async def test_async_pipeline():
        ...
        signals = await get_signals_from_scrapers_async(
            "Test market analysis", chart_image_b64=None, enable_premium=False
        )

        print(f"\nResults:")
        print(f"  Tickers: {len(signals.get('tickers', []))}")
        print(
            f"  Options sweeps: {signals.get('options_flow', {}).get('total_sweeps', 0)}"
        )
