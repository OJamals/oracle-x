...
class ProductionMLPipeline:
    ...
    def _setup_scheduler(self):
        ...
        logger.info(
            f"Scheduled training at {training_time}, monitoring every {monitor_interval}h"
        )
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            self.model_manager = create_ml_model_manager(
                models_dir="models/production", monitoring_db="models/monitoring.db"
            )
            ...
            self.model_manager.configure_auto_retraining(
                enabled=True, threshold_days=7, min_predictions=50
            )
    ...
    def _scheduled_training(self):
        ...
        try:
            ...
            results = self.model_manager.train_models(
                symbols=symbols, days_back=lookback_days, force_retrain=True
            )
            ...
            logger.info(
                f"Training completed: {successful_models}/{total_models} models successful"
            )
            ...
            self.performance_metrics["last_training"] = {
                ...
                "success_rate": (
                    successful_models / total_models if total_models > 0 else 0
                ),
        ...
        try:
            ...
            for model_name, status in all_status.items():
                ...
                if perf.get("accuracy", 1.0) < thresholds["accuracy"]:
                    alerts.append(
                        f"Model {model_name} low accuracy: {perf['accuracy']:.3f}"
                    )

                if perf.get("avg_error", 0.0) > thresholds["error_rate"]:
                    alerts.append(
                        f"Model {model_name} high error rate: {perf['avg_error']:.3f}"
                    )
            ...
            self.performance_metrics["monitoring"] = {
                ...
                "healthy_models": sum(
                    1 for s in all_status.values() if s["status"] == "healthy"
                ),
                "alerts": alerts,
            }
    ...
    def _health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.trading_orchestrator:
                ...
                health["components"]["trading_orchestrator"] = {
    ...
    def generate_predictions(
        ...
        predictions = {}
        prediction_types = [
            PredictionType[pt] for pt in self.config["prediction_types"]
        ]
        ...
        if self.model_manager:
            ...
            for symbol in self.config["symbols"]:
                for pred_type in self.config["prediction_types"]:
                    ...
                    model_data = self.model_manager.export_model_metrics(
                        model_name, days=30
                    )
                    if model_data:
...
def create_production_pipeline(
    config_path: str = "config/ml_production.json",
