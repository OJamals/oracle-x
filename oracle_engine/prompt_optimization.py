    ...
    def _load_default_templates(self):
        ...
        templates = [
            PromptTemplate(
                ...
                market_conditions=[MarketCondition.SIDEWAYS, MarketCondition.LOW_VOLUME],
...
""",
                priority_signals=["market_internals", "chart_analysis", "finviz_breadth"],
                signal_weights={
...
""",
                priority_signals=["options_flow", "dark_pools", "sentiment_web", "chart_analysis"],
                signal_weights={
                    "options_flow": 0.3,
                    "dark_pools": 0.2,
                    "sentiment_web": 0.2,
            ...
            PromptTemplate(
                ...
                market_conditions=[MarketCondition.EARNINGS, MarketCondition.NEWS_DRIVEN],
...
""",
                priority_signals=["earnings_calendar", "sentiment_web", "yahoo_headlines", "options_flow"],
                signal_weights={
...
""",
                priority_signals=["chart_analysis", "market_internals", "finviz_breadth"],
                signal_weights={
                    "chart_analysis": 0.4,
                    "market_internals": 0.3,
                    "finviz_breadth": 0.2,
    ...
    def classify_market_condition(self, signals: Dict[str, Any]) -> MarketCondition:
        ...
        try:
            ...
            # Sentiment analysis
            bullish_keywords = ["bullish", "optimistic", "positive", "rally", "breakout"]
            bearish_keywords = ["bearish", "pessimistic", "negative", "decline", "breakdown"]
            ...
            volatile_keywords = ["volatile", "uncertainty", "swing", "whipsaw", "erratic"]
            volatility_score = sum(1 for word in volatile_keywords if word in market_internals + sentiment_llm)
    ...
    def select_optimal_template(self, market_condition: MarketCondition, 
                              recent_performance: Optional[Dict] = None) -> PromptTemplate:
        """
        ...
        compatible_templates = [
            ...
            if market_condition in template.market_conditions or not template.market_conditions
        ]
        
        if not compatible_templates:
            # Fallback to default conservative template
            return self.prompt_templates.get("conservative_balanced", list(self.prompt_templates.values())[0])
        
        # If we have performance data, select based on historical success
        if recent_performance:
            best_template = max(compatible_templates, 
                              key=lambda t: recent_performance.get(t.template_id, {}).get('success_rate', 0.5))
            return best_template
        
        # Fallback: select based on strategy appropriateness
        strategy_priority = {
            MarketCondition.BULLISH: [PromptStrategy.AGGRESSIVE, PromptStrategy.MOMENTUM],
            MarketCondition.BEARISH: [PromptStrategy.CONTRARIAN, PromptStrategy.CONSERVATIVE],
            MarketCondition.VOLATILE: [PromptStrategy.AGGRESSIVE, PromptStrategy.TECHNICAL],
            MarketCondition.SIDEWAYS: [PromptStrategy.CONSERVATIVE, PromptStrategy.TECHNICAL],
            MarketCondition.EARNINGS: [PromptStrategy.FUNDAMENTAL, PromptStrategy.AGGRESSIVE],
            MarketCondition.NEWS_DRIVEN: [PromptStrategy.FUNDAMENTAL, PromptStrategy.MOMENTUM],
            MarketCondition.OPTIONS_HEAVY: [PromptStrategy.AGGRESSIVE, PromptStrategy.TECHNICAL],
            MarketCondition.LOW_VOLUME: [PromptStrategy.CONSERVATIVE, PromptStrategy.TECHNICAL]
        }
        
        preferred_strategies = strategy_priority.get(market_condition, [PromptStrategy.BALANCED])
    ...
    def optimize_signal_selection(self, signals: Dict[str, Any], 
        ...
        for signal_name, signal_data in signals.items():
            ...
            priority = weight * (1.0 if signal_name in template.priority_signals else 0.5)
        ...
        for signal_name, signal_data, priority in signal_priorities:
            ...
            if current_tokens + signal_tokens <= token_budget:
                ...
                if priority > 0.5:
                    compressed = self._compress_signal(signal_data, 
                                                     max_tokens=token_budget - current_tokens)
                    if compressed:
    ...
    def generate_optimized_prompt(self, signals: Dict[str, Any], 
                                 market_condition: Optional[MarketCondition] = None,
                                 template_id: Optional[str] = None) -> Tuple[str, str, Dict]:
        """
        ...
        user_prompt = template.user_prompt_template.format(signals_section=signals_section)
    ...
    def _format_signals_section(self, signals: Dict[str, Any], template: PromptTemplate) -> str:
        ...
        for signal_name, signal_data in signals.items():
            ...
            if signal_name == "market_internals":
                sections.append(f"**Market Internals** (Weight: {weight:.1f}):\n{signal_data}\n")
            elif signal_name == "options_flow":
                if isinstance(signal_data, list) and signal_data:
                    formatted = "\n".join([f"  - {item}" for item in signal_data[:5]])
                    sections.append(f"**Options Flow** (Weight: {weight:.1f}):\n{formatted}\n")
            elif signal_name == "sentiment_llm":
                sections.append(f"**LLM Sentiment** (Weight: {weight:.1f}):\n{signal_data}\n")
            elif signal_name == "chart_analysis":
                sections.append(f"**Chart Analysis** (Weight: {weight:.1f}):\n{signal_data}\n")
            else:
                # Generic formatting
                if isinstance(signal_data, (list, dict)):
                    formatted = str(signal_data)[:200] + ("..." if len(str(signal_data)) > 200 else "")
                else:
                    formatted = str(signal_data)[:200] + ("..." if len(str(signal_data)) > 200 else "")
                sections.append(f"**{signal_name.replace('_', ' ').title()}** (Weight: {weight:.1f}):\n{formatted}\n")
                
        return "\n".join(sections)
        
    def record_prompt_performance(self, prompt_metadata: Dict, 
        ...
        try:
            ...
            success_rate = sum(1 for attempt in model_attempts if attempt.get('success', False)) / len(model_attempts) if model_attempts else 0.0
            avg_latency = sum(attempt.get('latency_sec', 0) for attempt in model_attempts) / len(model_attempts) if model_attempts else 0.0
    ...
    def start_ab_test(self, template_a_id: str, template_b_id: str, 
                     market_condition: MarketCondition, duration_hours: int = 24) -> str:
        ...
        try:
            ...
            logger.info(f"Started A/B test {experiment_id}: {template_a_id} vs {template_b_id}")
    ...
    def evolve_prompts(self, top_performers: List[str], mutation_rate: float = 0.1) -> List[PromptTemplate]:
    ...
    def _mutate_template(self, template: PromptTemplate, mutation_rate: float) -> Optional[PromptTemplate]:
    ...
    def _mutate_signal_weights(self, template: PromptTemplate):
        ...
        for signal in template.signal_weights:
            if random.random() < 0.3:  # 30% chance to mutate each weight
                ...
                template.signal_weights[signal] = max(0.0, min(1.0, current_weight + mutation))
    ...
    def _mutate_prompt_structure(self, template: PromptTemplate):
        ...
        for old, new in variations:
            if random.random() < 0.2:  # 20% chance for each variation
                template.user_prompt_template = template.user_prompt_template.replace(old, new)
                
    def _mutate_context_compression(self, template: PromptTemplate):
        """Mutate context compression ratio"""
        mutation = random.uniform(-0.1, 0.1)
        template.context_compression_ratio = max(0.3, min(1.0, template.context_compression_ratio + mutation))
