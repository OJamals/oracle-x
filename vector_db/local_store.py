...
class VectorStore:
    ...
    def __init__(self):
        ...
        if not CHROMADB_AVAILABLE:
            reason = (
                f"ChromaDB unavailable; vector DB disabled ({_CHROMADB_IMPORT_ERROR})"
            )
    ...
    def _load_embedding_model(self) -> None:
        ...
        try:
            ...
            self._disable_vector_db(
                f"SentenceTransformers unavailable; vector DB disabled ({e})"
            )
            return
        ...
        try:
            ...
            self._disable_vector_db(
                f"Embedding model load failed; vector DB disabled ({e})"
            )
    ...
    def _initialize_client(self) -> None:
        ...
        if DISABLE_VECTOR_DB:
            self._mark_disabled(
                "ChromaDB disabled via ORACLEX_DISABLE_VECTOR_DB environment flag."
            )
            return
        ...
        if self._should_use_inmemory():
            try:
                ...
                print(
                    f"[WARN] In-memory ChromaDB failed, falling back to persistent: {e}"
                )
        ...
        if self.chroma_client is None:
            self._disable_vector_db(
                "ChromaDB unavailable; vector similarity features disabled."
            )
    ...
    def _try_legacy_client(
        self, storage_dir: Path, last_error: Optional[Exception] = None
    ) -> None:
    ...
    def store_trade_vector(self, trade: dict) -> bool:
        ...
        try:
            self.collection.upsert(
                ids=[point_id], embeddings=[vector], metadatas=[metadata]
            )
