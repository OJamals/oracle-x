...
def cmd_quote(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if preferred:
            ...
            quote = orch.get_orchestrator().get_quote(sym, preferred_sources=preferred)
        else:
            quote = orch.get_quote(sym)
        if not quote:
...
def cmd_market_data(args: argparse.Namespace) -> None:
    ...
    try:
        if preferred:
            md = orch.get_orchestrator().get_market_data(sym, period=period, interval=interval, preferred_sources=preferred)
        else:
            md = orch.get_market_data(sym, period=period, interval=interval)
...
def cmd_company_info(args: argparse.Namespace) -> None:
    sym = args.symbol.upper().strip()
    try:
        info = orch.get_company_info(sym)
        if not info:
...
def cmd_news(args: argparse.Namespace) -> None:
    ...
    limit = args.limit
    try:
        items = orch.get_news(sym, limit=limit) or []
...
def cmd_multiple_quotes(args: argparse.Namespace) -> None:
    ...
    try:
        results = orch.get_multiple_quotes(symbols) or {}
        norm = {}
...
def cmd_financial_statements(args: argparse.Namespace) -> None:
    sym = args.symbol.upper().strip()
    try:
        fin = orch.get_financial_statements(sym) or {}
        summary = {}
...
def cmd_sentiment(args: argparse.Namespace) -> None:
    sym = args.symbol.upper().strip()
    try:
        data = orch.get_sentiment_data(sym) or {}
...
def cmd_advanced_sentiment(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        sd = orch.get_advanced_sentiment(sym, texts=None, sources=None)
        if not sd:
...
def cmd_market_breadth(_: argparse.Namespace) -> None:
    try:
        breadth = orch.get_market_breadth()
        if not breadth:
...
def cmd_sector_performance(_: argparse.Namespace) -> None:
    try:
        groups = orch.get_sector_performance() or []
