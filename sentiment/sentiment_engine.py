...
try:
    ...
    logging.warning(
        "TextBlob not available. Sentiment will use VADER and FinBERT only."
    )
...
class FinancialLexicon:
    ...
    def __init__(self):
        ...
        self.bullish_terms = {
            ...
            "revenue growth": 1.3,
            "pricing power": 1.4,
            "backlog expanded": 1.2,
            "book-to-bill": 1.1,
            "beat and raise": 1.8,
            "reacceleration": 1.3,
            "visibility improved": 1.1,
        }
        ...
        self.bearish_terms = {
            ...
            "inventory overhang": -1.2,
            "order cancellations": -1.3,
            "probe": -1.1,
            "settlement": -0.6,
            "class action": -1.2,
            "regulatory risk": -1.1,
            "weak sell-through": -1.0,
            "credit losses": -1.1,
            "liquidity stress": -1.2,
        }
    ...
    def _apply_qualifiers(
        self, text_tokens: List[str], base_weight: float, hit_index: int
    ) -> float:
        window = 3
        w = base_weight
            ...
            for term, weight in terms_dict.items():
                if term in text_lower:
                    try:
                        ...
                        hit_index = (
                            tokens.index(term_first) if term_first in tokens else 0
                        )
                    except Exception:
                        hit_index = 0
...
class FinBERTAnalyzer:
    ...
    def __init__(self, model_name: str = "ProsusAI/finbert"):
        ...
        self.model_performance = {
            "finbert_news": {
                "accuracy": 0.87,
                "latency": 1.3,
                "financial_relevance": 0.94,
            },
            "finbert_earnings": {
                "accuracy": 0.89,
                "latency": 1.4,
                "financial_relevance": 0.96,
            },
            "finbert_prosus": {
                "accuracy": 0.82,
                "latency": 1.0,
                "financial_relevance": 0.88,
            },
            "fintwitbert_sentiment": {
                "accuracy": 0.91,
                "latency": 1.3,
                "financial_relevance": 0.98,
            },  # High relevance for Twitter
        }
    ...
    def _select_model_for_source(self, source: str) -> str:
        ...
        if (
            "twitter" in source_lower
            or "tweet" in source_lower
    ...
    def _ensure_correct_model(self, source: str):
        ...
        if self.model_name != self.model_options[preferred_model]:
            logger.info(
                f"Switching model from {self.model_name} to {self.model_options[preferred_model]} for source: {source}"
            )
    ...
    def _load_model_with_retry(
        self, model_name: str, max_retries: int = 5
    ) -> Tuple[Optional[Any], Optional[Any]]:
        """Load model with enhanced retry logic for rate limiting"""
        for attempt in range(max_retries + 1):
            try:
                logger.info(
                    f"Loading model {model_name} (attempt {attempt + 1}/{max_retries + 1})"
                )
                ...
                if is_rate_limit and attempt < max_retries:
                    ...
                    logger.warning(
                        f"Rate limit hit for {model_name}, retrying in {backoff_time}s (attempt {attempt + 1}/{max_retries + 1})"
                    )
                    ...
                    logger.warning(
                        f"Model load failed for {model_name}: {e}, retrying in {backoff_time}s"
                    )
                    time.sleep(backoff_time)
                    continue
                else:
                    logger.error(
                        f"Failed to load model {model_name} after {max_retries + 1} attempts: {e}"
                    )
                    return None, None
        ...
        try:
            ...
            with self._load_lock:
                ...
                if self.tokenizer is None or self.model is None:
                    logger.warning(
                        f"Primary model {self.model_name} failed to load, trying fallbacks"
                    )

                    # Try fallback models in order of performance
                    performance_ranked = sorted(
                        self.model_options.items(),
                        key=lambda x: self.model_performance.get(x[0], {}).get(
                            "accuracy", 0
                        ),
                        reverse=True,
                    )
                ...
                logger.info(
                    f"[TIMING][advanced_sentiment][finbert_load] {load_elapsed:.2f} seconds to load FinBERT"
                )
            ...
            try:
                ...
                with self._inference_lock:
                    ...
                    if os.getenv("ADV_FINBERT_VERBOSE", "0") == "1":
                        logger.info(
                            f"[TIMING][advanced_sentiment][finbert_tokenize] {enc_elapsed:.2f} seconds"
                        )
                ...
                with self._instance_lock, torch.no_grad():
                    ...
                    if os.getenv("ADV_FINBERT_VERBOSE", "0") == "1":
                        logger.info(
                            f"[TIMING][advanced_sentiment][finbert_infer] {model_elapsed:.2f} seconds"
                        )
                    logits = (
                    ...
                    if (
                        ...
                        if (
                            label_mapping.get(0) == "NEUTRAL"
                            and label_mapping.get(1) == "BULLISH"
                        ):
                            ...
                            label_mapping.get(0) == "LABEL_0"
                            and label_mapping.get(1) == "LABEL_1"
                        ):
                ...
                sentiment_score = self._apply_financial_context_boost(
                    text, sentiment_score, confidence
                )
                ...
                if is_rate_limit:
                    ...
                    logger.info(
                        f"Rate limit detected, using extended backoff: {backoff}s"
                    )
                else:
                    backoff = min(
                        0.5 * (2**retries), 2.0
    ...
    def _apply_financial_context_boost(
        self, text: str, sentiment_score: float, confidence: float
    ) -> float:
...
class AdvancedSentimentEngine:
    ...
    def analyze_text(
        self, text: str, symbol: str, source: str = "unknown"
    ) -> SentimentResult:
        """
        ...
        if self.textblob_available and TextBlob is not None:
            try:
                ...
                textblob_sentiment = getattr(
                    getattr(blob, "sentiment", None), "polarity", None
                )
            except Exception as e:
        ...
        try:
            lexicon_score, lexicon_matches = self.financial_lexicon.get_lexicon_score(
                text
            )
        ...
        try:
            finbert_sentiment, finbert_confidence = self.finbert_analyzer.analyze(
                text, source
            )
        except Exception as e:
        ...
        weights = self._get_dynamic_weights(
            len(text), lexicon_matches, finbert_confidence
        )
        ...
        if os.getenv("ADV_SENTIMENT_VERBOSE", "0") == "1":
            logger.info(
                f"[TIMING][advanced_sentiment][analyze_text] {processing_time:.2f}s for {symbol} source={source}"
            )
            print(
                f"[TIMING][advanced_sentiment][analyze_text] {processing_time:.2f}s for {symbol} source={source}"
            )
    ...
    def analyze_batch(
        ...
        if finbert_needed:
            try:
                ...
                if (
                    self.finbert_analyzer.tokenizer is None
                    or self.finbert_analyzer.model is None
                ):
                    raise RuntimeError(
                        "FinBERT model/tokenizer not available for batch inference"
                    )
                for i in range(0, len(finbert_needed), batch_size):
                    ...
                    sources_in_chunk = [
                        intermediate[idx]["source"]
                        for idx in chunk_indices
                        if idx < len(intermediate)
                    ]
                    dominant_source = (
                        max(set(sources_in_chunk), key=sources_in_chunk.count)
                        if sources_in_chunk
                        else "unknown"
                    )
                ...
                logger.warning(
                    f"FinBERT batch inference failed, falling back to per-text: {e}"
                )
                for orig_idx, text in finbert_needed:
                    # Get source from intermediate data
                    source = (
                        intermediate[orig_idx]["source"]
                        if orig_idx < len(intermediate)
                        else "unknown"
                    )
        ...
        for idx, meta in enumerate(intermediate):
            ...
            numerator = (
                vader_compound * vader_w
                + finbert_score * finbert_w
                + (lex_score if lex_matches > 0 else 0.0) * lex_w
            )
            ...
            confidence = max(
                ...
                min(
                    0.99,
                    (
                        abs(finbert_score) * 0.5
                        + abs(vader_compound) * 0.3
                        + min(1.0, lex_matches / 8.0) * 0.2
                    ),
                ),
            )
            processing_time = time.time() - start_time
            ...
            if os.getenv("ADV_SENTIMENT_VERBOSE", "0") == "1":
                logger.info(
                    f"[TIMING][advanced_sentiment][analyze_text_mb] {processing_time:.2f}s for {symbol} source={source}"
                )

        batch_elapsed = time.time() - batch_start
        logger.info(
            f"[TIMING][advanced_sentiment][analyze_batch] {batch_elapsed:.2f}s for {len(texts)} texts"
        )
        if os.getenv("ADV_SENTIMENT_VERBOSE", "0") == "1":
            print(
                f"[TIMING][advanced_sentiment][analyze_batch] {batch_elapsed:.2f}s for {len(texts)} texts"
    ...
    def get_symbol_sentiment_summary(
        ...
        weighted_sentiment = sum(
            score * conf for score, conf in zip(ensemble_scores, confidences)
        )
        ...
        trending_direction = self._determine_trend(
            overall_sentiment, avg_confidence, bullish, bearish, neutral
        )
    ...
    def _get_dynamic_weights(
        self, text_length: int, lexicon_matches: int, finbert_confidence: float
    ) -> Dict[str, float]:
    ...
    def _calculate_quality_score(
        self, sentiment_results: Dict[str, SentimentResult], average_confidence: float
    ) -> float:
        """Calculate quality score based on source diversity and confidence"""
        source_diversity_bonus = min(
            20, len(sentiment_results) * 3
        )  # Max 20 points for source diversity
        confidence_score = average_confidence * 60  # Max 60 points for confidence
        sample_size_bonus = min(
            20, sum(sd.sample_size or 0 for sd in sentiment_results.values()) / 10
        )  # Max 20 points for sample size
    ...
    def _calculate_enhanced_quality_score(
        self, results: List[SentimentResult], avg_confidence: float
    ) -> float:
        ...
        model_scores = [
            r.ensemble_score for r in results if abs(r.ensemble_score) > 0.01
        ]
        ...
        content_score = min(
            10.0, max(0.0, (avg_text_length - 50) / 5.0)
        )  # Bonus for longer, relevant content
...
def analyze_symbol_sentiment(
    symbol: str, texts: Optional[List[str]], sources: Optional[List[str]] = None
) -> SentimentSummary:
...
def analyze_text_sentiment(
    text: str, symbol: str, source: str = "unknown"
) -> SentimentResult:
