...
def check_service_health(url, api_key=None):
    ...
    if "vector" in url.lower() or "local" in url.lower():
        ...
        try:
            ...
            print(
                f"[DEBUG] Local vector store health check: SUCCESS, {stats.get('total_vectors', 0)} vectors"
            )
            return True
    ...
    try:
        ...
        print(
            f"[DEBUG] {url} status_code: {resp.status_code}, response: {resp.text[:200]}"
        )
...
if os.environ.get("STREAMLIT") or __name__ == "__main__":
    ...
    if submitted:
        with st.spinner("Running Oracle-X pipeline. This may take a few moments..."):
            try:
                print(
                    "[DEBUG] Submitting to run_oracle_pipeline with prompt:",
                    st.session_state["prompt_text"],
                )
                if not callable(run_oracle_pipeline):
                    st.error(
                        "Pipeline runner is not available in this environment. Start the dashboard with `streamlit run dashboard/app.py`."
                    )
                ...
                if tape:
                    st.markdown(
                        f"<div style='background:#f0f2f6;padding:1em;border-radius:8px;'><b>Tomorrow's Tape:</b> {tape}</div>",
    ...
    <div style='border:1px solid #e0e0e0; border-radius:10px; margin:1em 0; padding:1.5em; background:#fff;'>
      ...
      <div style='margin-bottom:0.5em;'><b>Scenario Probabilities:</b> """,
                            unsafe_allow_html=True,
                        )
                        ...
                        for case, text in scenario_tree.items():
                            ...
                            st.markdown(
                                f"<span style='background:{case_color};color:#fff;padding:0.2em 0.7em;border-radius:5px;margin-right:0.5em;font-size:0.9em;'>{case.replace('_',' ').title()}: {text}</span>",
                                unsafe_allow_html=True,
                            )
                        ...
                        if price_chart and os.path.exists(price_chart):
                            chart_cols[0].image(
                                price_chart,
                                caption=f"{ticker} Price Chart",
                                use_container_width=True,
                            )
                        if scenario_chart and os.path.exists(scenario_chart):
                            chart_cols[1].image(
                                scenario_chart,
                                caption=f"{ticker} Scenario Chart",
                                use_container_width=True,
                            )
                        st.markdown("</div>", unsafe_allow_html=True)
                else:
                ...
                if trades and isinstance(trades, list) and len(trades) > 0:
                    ...
                    summary_data = [
                        {col: trade.get(col, "") for col in summary_cols}
                        for trade in trades
                    ]
